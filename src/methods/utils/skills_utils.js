    ...
    case "object_at_location":
      ...
      try {
        if (condition.location == "cart") {
          ...
          location = locations.filter(
            (location) => location._id == condition.location
          );
          if (location.length) {
      ...
      if (condition.object == "cart") {
        ...
        object = objects.filter(
          (object) => object._id.$oid == condition.object
        );
        if (object.length) {
      ...
      if (embolden != false) {
        ...
        summary =
          `${quantity ? quantity + " x " : ""}` + object + " at " + location;
      }
...
export function skillName(skill, locations, skills) {
  let filteredSkills = skills.filter(
    (skillz) => skillz._id.$oid == skill.skill_id.$oid
  );
...
export function parseCondition(cond, skills) {
  ...
  switch (cond.type) {
    case "object_at_location":
      if (cond.object == "cart") {
        desc = "Cart at '" + cond.location + "'";
      } else if (cond.location == "hand") {
        desc = "'" + cond.object + "' in Hand";
      } else {
        desc = "'" + cond.object + "' at '" + cond.location + "'";
      }
      break;
    case "stack_complete":
