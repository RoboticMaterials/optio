    ...
    switch (condition.type) {
        case 'object_at_location':
            ...
            if (condition.object == 'cart') {
                ...
                object = objects.filter(object => object.id.$oid == condition.object);
        ...
        case 'hil_complete':
            ...
            if (!!skills) {
                if (!!skill.skillId) {
                    name = skills.filter(s => s.id.$oid == skill.skillId.$oid);
...
export function skillName(skill, locations, skills) {
    ...
    let filteredSkills = skills.filter(skillz => skillz.id.$oid == skill.skillId.$oid)
