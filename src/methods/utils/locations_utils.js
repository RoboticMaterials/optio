 ...
 */
export const compareExistingVsIncomingLocations = (
  incomingLocations,
  existingLocations,
  d3
) => {
  Object.values(existingLocations).forEach((existingLocation) => {
    ...
    if (existingLocation._id in incomingLocations) {
      Object.assign(incomingLocations[existingLocation._id], {
  ...
  Object.values(incomingLocations).forEach((incomingLocation) => {
    ...
    if (
      !!incomingLocation.change_key &&
      incomingLocation.change_key === "deleted"
    ) {
    ...
    if (!incomingLocation._id in existingLocations) {
      ...
      [x, y] = convertRealToD3(
        [incomingLocation.pos_x, incomingLocation.pos_y],
        d3
      );
      incomingLocation = {
        ...incomingLocation,
