...
export const generateDefaultRoute = (obj) => {
  ...
  return {
    ...defaultTask,
    device_types: MiRMapEnabled
      ? [DEVICE_CONSTANTS.MIR_100, DEVICE_CONSTANTS.HUMAN]
      : [DEVICE_CONSTANTS.HUMAN],
    handoff: true,
...
export const isHumanTask = (task) => {
  return (
    task &&
    isArray(task.device_types) &&
    task.device_types.includes(DEVICE_CONSTANTS.HUMAN)
  );
};
...
export const isMiRandHumanTask = (task) => {
  return (
    task &&
    isArray(task.device_types) &&
    task.device_types.includes(DEVICE_CONSTANTS.MIR_100) &&
    task.device_types.includes(DEVICE_CONSTANTS.HUMAN)
  );
};
...
export const isStationInRoutes = (routes, stationId) => {
  ...
  for (const currRoute of routes) {
    ...
    if (loadStationId === stationId || unloadStationId === stationId)
