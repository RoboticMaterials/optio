...
export default function dashboardsReducer(state = defaultState, action) {
  ...
  switch (action.type) {
    case SET + DASHBOARD + KICK_OFF_ENABLED: {
      const { dashboardId, kickOffEnabled } = action.payload;

      return {
        ...state,
        kickOffEnabledDashboards: {
          ...state.kickOffEnabledDashboards,
          [dashboardId]: kickOffEnabled,
        },
      };
    }

    case SET + DASHBOARD + FINISH_ENABLED: {
      const { dashboardId, finishEnabled } = action.payload;

      return {
        ...state,
        finishEnabledDashboards: {
          ...state.finishEnabledDashboards,
          [dashboardId]: finishEnabled,
        },
      };
    }
