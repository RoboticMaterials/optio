  ...
  const onUpdateStation = (station) => {
    return {
      ...
      selectedStation:
        ...
        state.selectedStation._id === station._id &&
        station,
      pending: false,
    };
  };
  ...
  switch (action.type) {
    ...
    case SET_STATION_ATTRIBUTES:
      ...
      if (
        ...
        action.payload.id === state.selectedStation._id
      ) {
        return {
          ...state,
          selectedStation: {
    ...
    case UPDATE_STATIONS:
      return {
        ...
        selectedStation: action.payload.selectedStation
          ? action.payload.selectedStation
          : state.selectedStation,
    ...
    case GET_STATIONS_SUCCESS:
      const parsedStations = compareExistingVsIncomingLocations(
        deepCopy(action.payload),
        deepCopy(state.stations),
        state.d3
      );
