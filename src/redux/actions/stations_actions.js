...
export const getStations = () => {
  return async (dispatch) => {
    ...
    try {
      ...
      const normalizedStations = normalize(stations, stationsSchema)?.entities
        ?.stations
        ? normalize(stations, stationsSchema)?.entities?.stations
        : {};
...
const onRemoveStation = (id) => {
  ...
  let station = stationsState.selectedStation
    ? stationsState.selectedStation
    : stationsState.stations[id];
  ...
  return async (dispatch) => {
    ...
    // Else delete in backend and delete any associated tasks
      ...
      Object.values(tasks)
        .filter((task) => {
          return (
            task.load.station === station._id ||
            task.unload.station === station._id
          );
        })
...
const onSaveChildren = () => {
  return async (dispatch) => {
    const positionsState = store.getState().positionsReducer;
    const selectedStationChildrenCopy =
      positionsState.selectedStationChildrenCopy;
