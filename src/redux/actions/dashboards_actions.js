...
import { DASHBOARD, DASHBOARDS } from "../types/data_types";

import { SET } from "../types/prefixes";

import { KICK_OFF_ENABLED, FINISH_ENABLED } from "../types/suffixes";
...
export const removeRouteFromAllDashboards = (routeId) => {
  return async (dispatch, getState) => {
    ...
    Object.values(dashboards)
      .filter(
        (dashboard) =>
          dashboard.station === selectedTask?.load?.station ||
          dashboard.station === selectedTask?.unload?.station
      )
      .forEach((currDashboard) => {
        var currButtons = [...currDashboard.buttons];

        currButtons = currButtons.filter(
          (button) => button.task_id !== routeId
        );
...
export const addRouteToDashboards = (route) => {
  return async (dispatch, getState) => {
    ...
    const { _id: routeId, type: routeType, name: routeName } = route;
...
export const removeRouteFromWrongDashboards = (route) => {
  return async (dispatch, getState) => {
    ...
    const { _id: routeId, type: routeType } = route;
    ...
    dashboards.forEach((currDashboard) => {
      ...
      const { $oid: currDashboardId } = currDashboardIdObj;
      ...
      if (currStationId !== stationId) {
        ...
        const filteredButtons = currDashboardButtons.filter(
          (currButton, currButtonIndex) => {
            const { task_id: currRouteId } = currButton;
...
export const setDashboardKickOffProcesses = (dashboardId, kickOffEnabled) => {
  return {
    type: SET + DASHBOARD + KICK_OFF_ENABLED,
    payload: { dashboardId, kickOffEnabled },
  };
};

export const setDashboardFinishProcesses = (dashboardId, finishEnabled) => {
  return {
    type: SET + DASHBOARD + FINISH_ENABLED,
    payload: { dashboardId, finishEnabled },
  };
};
