...
const TaskQueueListItem = (props) => {
  // extract props
  const { id, item, task, onClick, type, ...rest } = props;
  ...
  const selectedTask =
    useSelector((state) => state.tasksReducer.selectedTask) || {};
  const { _id: selectedTaskId } = selectedTask;
  ...
  return (
    <style.ItemDiv>
      <BasicListItem
        ...
        rightContent={
          <>
            <IconButton color={themeContext.fg.primary}>
              {type == "human" ? (
            ...
            <IconButton color={themeContext.fg.primary} onClick={handleClick}>
              <style.StyledRemoveIcon fontSize={"large"} />
            </IconButton>
          </>
        }
      />
