...
const HILModals = (props) => {
  const { hilMessage, hilType, taskQuantity, taskQueueID, item } = props;

  const { dashboard: dashboardId } = item || {};

  const dispatch = useDispatch();
  const dispatchGetCards = () => dispatch(getCards());
  const dispatchTaskQueueItemClicked = (id) =>
    dispatch({ type: "TASK_QUEUE_ITEM_CLICKED", payload: id });
  const disptachHILResponse = (response) =>
    dispatch({ type: "HIL_RESPONSE", payload: response });
  const disptachPutTaskQueue = async (item, id) =>
    await dispatch(putTaskQueue(item, id));
  const dispatchSetActiveHilDashboards = (active) =>
    dispatch({ type: "ACTIVE_HIL_DASHBOARDS", payload: active });
  const dispatchLocalHumanTask = (bol) =>
    dispatch({ type: "LOCAL_HUMAN_TASK", payload: bol });

  const hilTimers = useSelector((state) => {
    return state.taskQueueReducer.hilTimers;
  });
  const tasks = useSelector((state) => {
    return state.tasksReducer.tasks;
  });
  const taskQueue = useSelector((state) => state.taskQueueReducer.taskQueue);
  const activeHilDashboards = useSelector(
    (state) => state.taskQueueReducer.activeHilDashboards
  );
  const taskQueueItemClicked = useSelector(
    (state) => state.taskQueueReducer.taskQueueItemClicked
  );
  const dashboardOpen = useSelector(
    (state) => state.dashboardsReducer.dashboardOpen
  );
  const dashboards =
    useSelector((state) => state.dashboardsReducer.dashboards) || {};
  ...
  const { type, load, unload } = currentTask || {};

  const { station: unloadStationId } = unload || {};

  const { station: loadStationId } = load || {};

  const { name: selectedLotName, _id: selectedLotId, bins } = selectedLot || {};

  const currentBin = bins ? bins[loadStationId] : {};
  const { count } = currentBin || {};
  ...
  useEffect(() => {
    ...
    const routeProcesses =
      getRouteProcesses(currentTask?._id).map(
        (currProcess) => currProcess._id
      ) || [];
   ...
   * */
  useEffect(() => {
    const taskProcesses = getRouteProcesses(selectedTask?._id).map(
      (currProcess) => currProcess._id
    );

    if (
      taskProcesses &&
      Array.isArray(taskProcesses) &&
      taskProcesses.length > 0
    ) {
      setIsProcessTask(true);

      let stationCards = Object.values(cards).filter((currCard) => {
        const { bins, process_id: currCardProcessId } = currCard || {};

        if (bins) {
          if (
            bins[loadStationId] &&
            bins[loadStationId].count > 0 &&
            taskProcesses.includes(currCardProcessId)
          )
            return true;
        }
      });
  ...
  useEffect(() => {
    ...
    if (currentTask) {
      if (currentTask.associated_task)
        setAssociatedTask(tasks[currentTask.associated_task]);
    }

    // If the task's load location of the task q item matches the item's location then its a load hil, else its unload
    if (
      (currentTask && currentTask?.load?.station === item.hil_station_id) ||
      !!item.dashboard
    ) {
      // load
  ...
  const onHilSuccess = async (fraction) => {
    ...
    dispatchSetActiveHilDashboards(
      delete activeHilDashboards[item.hil_station_id]
    );
        ...
        {windowWidth < 700 ? (
          <>
            {taskHasProcess && ( //If the task isn't part of a process don't render the choose lot stuff on the HIL
              ...
              >
                {selectedLot ? (
                  <styled.LotTitleDescription style={{ fontSize: "1.5rem" }}>
                    Selected Lot:
                  </styled.LotTitleDescription>
                ) : (
                  <styled.FooterButton
                    onClick={() => {
                      setShowLotSelector(true);
      ...
      <>
        ...
        <styled.LotSelectorContainer>
          <styled.LotsContainer>
            ...
            >
              <styled.HilIcon className="fas fa-check" color={"#1c933c"} />
              ...
              return (
                ...
                >
                  ...
                  <styled.RowContainer>
                    ...
                    {!!count && (
                      <styled.HilButtonQuantityText color={"#1c933c"}>
                        {"(Quantity " + Math.ceil(count * decimal) + ")"}
                      </styled.HilButtonQuantityText>
                    )}
                  </styled.RowContainer>
                </styled.HilButton>
              );
    ...
    return (
      <>
        ...
        <styled.LotSelectorContainer>
          <styled.LotsContainer
            style={{
              justifyContent: hilLoadUnload === "load" && "space-between",
            }}
          >
            <div
              style={{
            ...
            >
              {(hilType === "pull" || hilType === "push") &&
                hilLoadUnload === "load" && (
                  <>
                    <styled.HilMessage style={{ marginBottom: "1rem" }}>
                      Enter Quantity
                    </styled.HilMessage>
                    ...
                    <styled.HilInputContainer>
                      {windowWidth > 590 && (
                        ...
                        >
                          <styled.HilInputIcon
                            ...
                            style={{ color: "#ff9898" }}
                          />
                          <styled.HilInputIconText style={{ color: "#ff9898" }}>
                            10
                          </styled.HilInputIconText>
                        </styled.HilInputIconContainer>
                      )}
                      ...
                      <styled.HilInput
                        ...
                        value={quantity}
                        style={
                          quantity > count ? { backgroundColor: "red" } : {}
                        }
                      ...
                      >
                        <styled.HilInputIcon
                          ...
                          style={{ color: "#90eaa8" }}
                        />
                        <styled.HilInputIconText style={{ color: "#90eaa8" }}>
                          1
                        </styled.HilInputIconText>
                      </styled.HilInputIconContainer>
              ...
              {count && (
                <styled.HilSubText style={{ marginBottom: "1rem" }}>
                  Available Lot Items: {count}
                </styled.HilSubText>
              )}
            </div>
            ...
            <styled.HilButtonContainer>
              ...
              {((hilType === "pull" && hilLoadUnload === "load") ||
                hilType === "check") && (
                ...
                hilLoadUnload === "load" && (
                  <styled.HilButton color={"#ff9898"} onClick={onHilFailure}>
                    <styled.HilIcon
                      ...
                      color={"#ff1818"}
                    />
                    <styled.HilButtonText color={"#ff1818"}>
                      Cancel
                    </styled.HilButtonText>
                  </styled.HilButton>
                )}
      ...
      <>
        ...
        <styled.LotSelectorContainer>
            ...
            <styled.LotsContainer>
              {availableLots
                ...
                .map((currLot, lotIndex) => {
                  ...
                  const objectName = objects[object_id]
          ...
          <styled.FooterContainer>
            ...
            <styled.FooterButton color={"#ff9898"} onClick={onHilFailure}>
              <styled.HilIcon
                className="fas fa-times"
                color={"#ff1818"}
