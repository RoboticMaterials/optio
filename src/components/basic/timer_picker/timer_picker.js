...
import {
  ...
  minutesRegex,
  secondsRegex,
} from "../../../methods/utils/regex_utils";
...
const TimePicker = (props) => {
  ...
  const [showMilitaryTime, setShowMilitaryTime] = useState(
    defaultShowMilitaryTime
  );
  ...
  return (
    <styled.Container>
      <styled.ColumnsContainer>
        {showHours && renderColumn(COLUMNS.HOURS)}
        {showMinutes && renderColumn(COLUMNS.MINUTES)}
        {showSeconds && renderColumn(COLUMNS.SECONDS)}
      </styled.ColumnsContainer>
    </styled.Container>
  );
};
