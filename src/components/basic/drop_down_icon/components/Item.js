...
class Item extends Component {
  ...
  render() {
    ...
    const { props, state, methods, item, itemIndex, ItemComponent, ContentContainer, icon, ButtonComponent, TextComponent, onMouseEnter, onMouseLeave, setFieldType } = this.props;
    ...
    return (
      <ItemComponent
        ...
        onClick={() => {
          methods.clearAll()
          props.setFieldType(item.type, item.dataType)
        }}
        color={props.color}
        schema={props.schema}
        onMouseEnter = {() => {
        ...
        >
        <i class = {icon} style = {{fontSize: '1rem', color: '#7e7e7e', marginRight: '1.5rem', marginTop: '0.2rem'}}/>
        <TextComponent>
