...
const StreamContainer = (props) => {
  const { status, error, myPeerId, loading, streams } = props;
    ...
    <styled.PlayerWrapper>
      <styled.TextContainer>
        <ClipLoader
          ...
          loading={loading}
        />
        {status && <styled.StatusText>{status}</styled.StatusText>}

        {error && <styled.ErrorText>{error}</styled.ErrorText>}
      </styled.TextContainer>

      <styled.VideoContainer id="stream" autoPlay playsInline muted>
