...
const DashboardEditor = (props) => {
  let { dashboard, setShowSidebar, showSidebar } = props;
  ...
  const onPutDashboard = (dashboard, id) =>
  ...
  const getInitialErrors = () => {
    ...
    try {
      // try / catch in case dashboard is null or doesn't have buttons
      const buttons = dashboard.buttons; // get buttons
  ...
  const handleSubmit = async (values) => {
    ...
    history.push(
      `/locations/${params.stationID}/dashboards/${params.dashboardID}/`
    );
  };
    ...
    >
      {(formikProps) => {
        ...
        const handleDrop = (dropResult) => {
          ...
          if (payload === null) {
            //  No new button, only reorder
            const shiftedButtonsCopy = arrayMove(
              buttonsCopy,
              removedIndex,
              addedIndex
            );
        ...
        const handleDeleteDashboard = async () => {
          ...
          const deleteDashboardIndex = stationCopy.dashboards.indexOf(
            params.dashboardID
          );
