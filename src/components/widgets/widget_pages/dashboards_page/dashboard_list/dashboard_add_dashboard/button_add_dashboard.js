      ...
      <>
        <style.InternalContainer className="col-xl-3 col-lg-4 col-md-6">
          <style.AddDashboardButton
            onClick={() => this.props.initializeNewDashboard()}
          >
...
const mapDispatchToProps = (dispatch) => {
  return {
    onAddButtons: (currentDashboardInfo) =>
      dispatch({ type: "BUTTONS", currentDashboard: currentDashboardInfo }),
    onDashboardAdd: (newDashboard) => dispatch(postDashboards(newDashboard)),
    onDashboardGet: () => dispatch(getDashboards()),
    onAddingDashboard: (value) =>
      dispatch({ type: "ADDING_DASHBOARD", payload: value }),
    onDashboardIdClicked: (Id) =>
      dispatch({ type: "POST_ID_DASHBOARD", ID: Id }),
    onDashboardTasksEdit: (value) =>
      dispatch({ type: "EDITING_TASKS", payload: value }),
    onSelectedDashboard: (value) =>
      dispatch({ type: "CURRENT_DASHBOARD", payload: value }),
  };
};
