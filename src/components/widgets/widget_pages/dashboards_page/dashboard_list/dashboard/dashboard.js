...
const Dashboard = React.memo((props) => {
  ...
  React.useEffect(() => {
    listRef.current && listRef.current.scrollTo(buttons.length);
  }, [buttons.length]);

  return (
    <style.Container ref={dropRef} onClick={props.openDashboard}>
        ...
        {buttons && buttons.length > 0 ? (
          <ReactList
            ...
            itemsRenderer={(items, ref) => {
              return (
                <style.ListContainer ref={ref}>{items}</style.ListContainer>
              );
            }}
          />
        ) : (
