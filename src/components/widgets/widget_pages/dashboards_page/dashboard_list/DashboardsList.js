...
const DashboardsList = (props) => {
  ...
  const onPutDashboard = (dashboard, copy, id) =>
    dispatch(dashboardActions.putDashboard(dashboard, copy, id));
  const onPostDashboard = (dashboard) =>
    dispatch(dashboardActions.postDashboard(dashboard));
  const onPutStation = (station, id) =>
    dispatch(stationActions.putStation(station, id));
  const onDeleteDashboard = (dashboard) =>
    dispatch(dashboardActions.deleteDashboard(dashboard));
  ...
  const dashboardsArray = selectedDashboardType
    ? selectedDashboardType.dashboards
      ? selectedDashboardType.dashboards.map(
  ...
  const handleNew = (button) => {
    ...
    postDashboardPromise.then(async (postedDashboard) => {
      ...
      await dispatch(
        stationActions.setStationAttributes(station._id, {
          dashboards: stationDashboards,
        })
      );
  ...
  return (
    <style.Container>
      ...
      <ConfirmDeleteModal
        ...
        textMain={
          dashboards[dashboardToDelete] &&
          'Are you sure you want to delete "' +
            dashboards[dashboardToDelete].name +
            '"'
        }
        caption={""}
