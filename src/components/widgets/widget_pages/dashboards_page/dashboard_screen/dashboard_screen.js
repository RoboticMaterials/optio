...
import {
  ADD_TASK_ALERT_TYPE,
  PAGES,
} from "../../../../../constants/dashboard_contants";
...
import {
  OPERATION_TYPES,
  TYPES,
} from "../dashboards_sidebar/dashboards_sidebar";
...
const DashboardScreen = (props) => {
  ...
  const currentDashboard = useSelector((state) => {
    return state.dashboardsReducer.dashboards[dashboardId];
  });
  const taskQueue = useSelector((state) => state.taskQueueReducer.taskQueue);
  const devices = useSelector((state) => state.devicesReducer.devices);
  const positions = useSelector((state) => state.positionsReducer.positions);
  const tasks = useSelector((state) => state.tasksReducer.tasks);
  const hilResponse = useSelector(
    (state) => state.taskQueueReducer.hilResponse
  );
  ...
  const onHILResponse = (response) =>
    dispatch({ type: "HIL_RESPONSE", payload: response });
  const onLocalHumanTask = (bol) =>
    dispatch({ type: "LOCAL_HUMAN_TASK", payload: bol });
  const onPutTaskQueue = async (item, id) =>
  ...
  const handleDashboardButtons = () => {
    ...
    // Else if the task q contains a human task that is unloading, show an unload button
      ...
      Object.values(taskQueue).map((item, ind) => {
        ...
        if (
          !!item.hil_station_id &&
          item.hil_station_id === stationID &&
          hilResponse !== item._id &&
          item?.device_type === "human"
        ) {
          buttons = [
            ...buttons,
            {
  ...
  return (
    ...
    >
      {reportModal === OPERATION_TYPES.REPORT.key && (
        <ReportModal
          ...
          onSubmit={(name, success) => {
            ...
            setAddTaskAlert({
              type: success
                ? ADD_TASK_ALERT_TYPE.REPORT_SEND_SUCCESS
                : ADD_TASK_ALERT_TYPE.REPORT_SEND_FAILURE,
      ...
      {reportModal === OPERATION_TYPES.KICK_OFF.key && (
        <KickOffModal
          ...
          onSubmit={(name, success) => {
            ...
            setAddTaskAlert({
              type: success
                ? ADD_TASK_ALERT_TYPE.KICK_OFF_SUCCESS
                : ADD_TASK_ALERT_TYPE.KICK_OFF_FAILURE,
              label: success
                ? "Lot Kick Off Successful"
                : "Lot Kick Off Failed",
              message: name ? `"` + name + `"` : null,
            });
      ...
      <TaskAddedAlert {...addTaskAlert} visible={!!addTaskAlert} />

      {mobileMode && <DashboardTaskQueue />}
    </style.Container>
  );
};
