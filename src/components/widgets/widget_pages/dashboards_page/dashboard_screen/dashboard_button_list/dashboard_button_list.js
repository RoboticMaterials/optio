...
const DashboardButtonList = (props) => {
  ...
  const itemRenderer = (index, key) => {
    ...
    const handleRouteClick = () => {
      disabled =
        addedTaskAlert || currentButton.deleted || broken || !taskExists;
      ...
      onClick = (associatedTaskIdArg, deviceType) => {
        if (taskID === "custom_task" || taskID === "hil_success") {
          onTaskClick(
            ...
            associatedTaskIdArg,
            name,
            currentButton.custom_task,
            deviceType ? deviceType : currentButton.deviceType
          );
        } else {
          onTaskClick(
            TYPES.ROUTES.key,
    ...
    <style.Container>
      <ReactList
        ...
        itemsRenderer={(items, ref) => {
          return <style.ListContainer ref={ref}>{items}</style.ListContainer>;
        }}
      />
    </style.Container>
  );
