...
const TaskAddedAlert = (props) => {
  // extract props
  const { type, visible, label, message, containerStyle } = props;
  ...
  if (visible) {
    return (
      <style.AddTaskAlertContainer style={containerStyle}>
        <style.AddTaskAlertLabel color={alertColor}>
          {label}
        </style.AddTaskAlertLabel>
        <style.AddTaskAlertMessage color={alertColor}>
          {message}
        </style.AddTaskAlertMessage>
