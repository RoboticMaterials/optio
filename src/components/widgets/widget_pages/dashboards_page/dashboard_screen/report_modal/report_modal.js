...
const NewButtonForm = (props) => {
  const { cancel, dashboard, buttonId, editing } = props;

  const report_buttons = dashboard?.report_buttons || [];

  const dispatch = useDispatch();
  const onPutDashboard = (dashboardCopy, dashboardId) =>
    dispatch(putDashboard(dashboardCopy, dashboardId));

  const editingButton = report_buttons.find(
    (currButton) => currButton._id === buttonId
  );
  ...
  return (
    ...
    >
      {(formikProps) => {
        ...
        const { color, label, iconClassName } = values;
...
const ReportModal = (props) => {
  ...
  const onPutDashboard = (dashboardCopy, dashboardId) =>
    dispatch(putDashboard(dashboardCopy, dashboardId));
  const onPostReportEvent = (reportEvent) =>
    dispatch(postReportEvent(reportEvent));
  const onPutReportEvent = (id, reportEvent) =>
    dispatch(putReportEvent(id, reportEvent));
  ...
  const reportEvents = useSelector((state) => {
    return state.reportEventsReducer.reportEvents;
  });

  const sendReport = async (button) => {
    setSubmitting(true);
    const { _id, iconClassName, color, ...rest } = button;
        ...
        {addingNew || sending ? (
          ...
          >
            <styled.ContentContainer>
              {editing && (
                <styled.AddNewButtonContainer showBorder={!noButtons}>
                  <Button
                          ...
                          <div
                            ...
                            onClick={(event) => {
                              ...
                              const updatedDashboard = {
                                ...
                                report_buttons: report_buttons.filter(
                                  (currOldButton) => currOldButton._id !== _id
                                ),
                              };
