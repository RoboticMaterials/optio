          ...
          {Object.values(cards).map((card, ind) => (
            <>
              {!!card.bins[location._id] && (
                <style.LotItem>
                  {card.name + " (" + card.bins[location._id].count + ")"}
                </style.LotItem>
              )}
            </>
          ))}
    ...
    <style.ColumnContainer>
      ...
      <style.Header>
        {showBackButton && (
          <BackButton
            style={{ order: "1" }}
            containerStyle={{ marginTop: "1.8rem" }}
            onClick={onBack}
          />
        )}

        {showTitle && <style.Title style={{ order: "2" }}>{page}</style.Title>}

        {showEditButton && !mobileMode && (
          <Button
            style={{ order: "3", marginTop: "1.8rem" }}
