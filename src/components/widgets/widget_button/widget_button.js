...
const WidgetButton = (props) => {
  const { type, currentPage, id, coordinateMove, label, toggle } = props;
  ...
  const dispatchHandlePostTaskQueue = (props) =>
    dispatch(handlePostTaskQueue(props));
    ...
    >
      {type === "view" ? (
        ...
        <>
          <styled.WidgetButtonIcon
            style={{
              fontSize: type === "cart" && "1.2rem",
              paddingTop: type === "cart" && ".8rem",
            }}
            className={"icon-" + type}
            pageID={type}
            currentPage={currentPage}
