...
const Widgets = (props) => {
  ...
  const editingPosition = useSelector(
    (state) => state.positionsReducer.editingPosition
  );
  ...
  const dispatchSetSelectedStationChildrenCopy = (locationChildren) =>
    dispatch(setSelectedStationChildrenCopy(locationChildren));
  ...
  const handleWidgetButtons = useMemo(() => {
    const location = stations[hoveringInfo.id]
      ? stations[hoveringInfo.id]
      : positions[hoveringInfo.id];
    ...
    if (location.schema === "station") {
      ...
      if (location.device_id) {
        ...
        if (DeviceItemTypes[device.device_model])
    ...
    return (
      <styled.WidgetStatisticsContainer>
        <styled.WidgetStatisticsBlock>
          ...
          <styled.WidgetStatisticsText>30s</styled.WidgetStatisticsText>
        </styled.WidgetStatisticsBlock>

        <styled.WidgetStatisticsBlock>
          <styled.WidgetStatisticsIcon className="fas fa-arrow-right" />
          <styled.WidgetStatisticsText>25 Units</styled.WidgetStatisticsText>
        </styled.WidgetStatisticsBlock>

        <styled.WidgetStatisticsBlock>
          <styled.WidgetStatisticsIcon className="fas fa-percent" />
          <styled.WidgetStatisticsText>98%</styled.WidgetStatisticsText>
        </styled.WidgetStatisticsBlock>
      </styled.WidgetStatisticsContainer>
    );
  }, [stations, positions]);
  ...
  const handleWidgetPosition = (coord) => {
    ...
    if (
      !!selectedPosition &&
      selectedPosition.schema === "temporary_position"
    ) {
  ...
  return (
    <>
      {!!widgetPage && <styled.WidgetBlurContainer />}
      ...
      >
        ...
        <styled.WidgetContainer
          widgetPage={widgetPage}
          type={!!selectedLocation && selectedLocation.type}
        >
          {!widgetPage && !!selectedLocation && (
            <>
              {selectedLocation.schema == "temporary_position" ? (
                <styled.WidgetStationName>
                  {"Send Cart To Location"}
                </styled.WidgetStationName>
              ) : (
                <>
                  {selectedLocation.parent ? (
                    <styled.WidgetPositionName>
