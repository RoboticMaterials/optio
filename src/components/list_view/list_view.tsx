...
const ListView = (props) => {
    ...
    const onLocationClick = async (item) => {
        const station = stations[item._id]
        let dashboardID = station?.dashboards?.[0]
        ...
        if (dashboardID === undefined) {
            ...
            const postedDashboard = await dispatchPostDashboard(defaultDashboard)
            dashboardID = postedDashboard._id?.$oid || postedDashboard._id
            const updatedStation = { ...station, dashboards: [dashboardID] }
            await dispatchPutStation(updatedStation)
        }

        history.push('/locations/' + item._id + '/dashboards/' + dashboardID)
        setShowDashboards(true)
        setTitle(station?.name + ' Dashboard')
    }
