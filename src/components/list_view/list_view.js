...
const ListView = (props) => {
  ...
  const CURRENT_SCREEN = showDashboards
    ? SCREENS.DASHBOARDS
    : showSettings
    ? SCREENS.SETTINGS
    : SCREENS.LOCATIONS;
  ...
  const handleTaskQueueStatus = () => {
    ...
    return Object.values(taskQueue).map((item, ind) => {
      ...
      if (item.owner) {
        ...
        if (
          !!devices[params.stationID] &&
          item.owner === devices[params.stationID]._id
        ) {
          let locationName = "";
          ...
          return (
            <styled.StatusContainer>
              <p>{`Distance to ${locationName} - ${Math.floor(
                devices[item.owner].distance_to_next_target
              )}m`}</p>
            </styled.StatusContainer>
          );
        }
    ...
    <styled.Container>
      <styled.Header>
        {showDashboards ? (
          ...
          >
            <styled.Icon className={"fa fa-times"} />
          </BounceButton>
        ) : (
          <BounceButton
            color={"black"}
