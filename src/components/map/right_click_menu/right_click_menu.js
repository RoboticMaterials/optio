...
const RightClickMenu = (props) => {
  const { coords, buttonClicked, d3 } = props;
  ...
  const onSendCartToPosition = async () => {
    ...
    await Object.assign(tempSelectedPosition, {
      ...PositionTypes["temporary_cart_position"].attributes,
      temp: true,
    });
  ...
  return (
    <styled.MenuContainer style={{ top: coords.y, left: coords.x }}>
      {MiRMapEnabled ? (
        <>
          <styled.MenuButton
            disabled={disbaleStation}
            onClick={() => !disbaleStation && onAddStation()}
          >
            Add Station
          </styled.MenuButton>
          <styled.MenuButton onClick={onSendCartToPosition}>
            Send Cart to Position
          </styled.MenuButton>
        </>
      ) : (
        <styled.MenuButton onClick={onAddStation}>
          Add Station
        </styled.MenuButton>
      )}
