...
function Position(props) {
  ...
  const hoveringID = useSelector(
    (state) => state.widgetReducer.hoverLocationID
  );
  ...
  if (
    ...
    positionId in selectedStationChildrenCopy &&
    !selectedTask
  )
    isSelected = true;
  // Set selected if there is a selected postion that is this position and no selected task
    ...
    !!selectedPosition &&
    selectedPosition._id === positionId &&
    !selectedTask
  )
    isSelected = true;
  ...
  if (!!selectedPosition && selectedPosition._id !== positionId)
    disabled = true;
  ...
  else if (!!selectedStation && !selectedStation.children.includes(positionId))
    disabled = true;
  ...
  // If the process is broken, then you can only start the task at the route before break's unload location
    ...
    if (!position.parent) {
      ...
      // setting unload
        ...
        if (isPositionAtLoadStation(routeAfterBreak, positionId))
          disabled = false;
      }
    }
  }
  ...
  // This eliminates process with gaps between stations
    ...
    if (!position.parent) {
      ...
      if (selectedProcess.routes.length > 0) {
        const routeIndex = getRouteIndexInRoutes(
          selectedProcess.routes.map((currProcess) => currProcess._id),
          selectedTask?._id
        );
        ...
        if (!routeStart || (routeStart && routeEnd)) {
          ...
          if (insertIndex === 0) {
            ...
            if (isPositionInRoutes(selectedProcess.routes, positionId))
              disabled = true;
            if (isPositionAtLoadStation(selectedTask, positionId))
              disabled = false;
          } else {
        ...
        // setting unload
          ...
          if (insertIndex === 0) {
            ...
            if (isPositionAtLoadStation(firstRoute, positionId))
              disabled = false;
            ...
            if (isPositionAtLoadStation(nextRoute, positionId))
              disabled = false;
          } else {
            // disable positions already used
            if (isPositionInRoutes(selectedProcess.routes, positionId))
  ...
  const shouldGlow =
    selectedTask !== null &&
    ((selectedTask.load.position == positionId &&
      selectedTask.type == "push") ||
      (selectedTask.unload.position == positionId &&
        selectedTask.type == "pull") ||
      (selectedTask.load.position == positionId &&
        selectedTask.type == "both") ||
      (selectedTask.unload.position == positionId &&
        selectedTask.type == "both"));
  ...
  const onSetPositionTask = () => {
    ...
    if (selectedTask !== null) {
      ...
      if (
        selectedTask.load.position !== null &&
  ...
  return (
    <React.Fragment key={`frag-loc-${positionId}`}>
      ...
      <LocationSvg
        ...
        rotating={rotating}
        hoveringInfo={
          position.schema !== "temporary_position" ? hoveringInfo : null
        }
      ...
      <DragEntityProto
        ...
        handleRotate={(rotation) => {
          dispatchSetPositionAttributes(positionId, { rotation });
        }}
        handleTranslate={({ x, y }) =>
          dispatchSetPositionAttributes(positionId, { x, y })
        }
        handleTranslateEnd={({ x, y }) => {
          const pos = convertD3ToReal([x, y], props.d3);
          dispatchSetPositionAttributes(positionId, {
            pos_x: pos[0],
            pos_y: pos[1],
          });
        }}
