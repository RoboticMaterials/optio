...
const DragEntityProto = (props) => {
  ...
  const rotate = (event, element) => {
    ...
    if (event.sourceEvent.type == "mousemove") {
      ...
      angle =
        (Math.atan2(
          event.sourceEvent.clientY - location.y,
          event.sourceEvent.clientX - location.x
        ) *
          180) /
        Math.PI;
    } else if (event.sourceEvent.type == "touchmove") {
      // Tablet
      angle =
        (Math.atan2(
          event.sourceEvent.touches[0].clientY - location.y,
          event.sourceEvent.touches[0].clientX - location.x
        ) *
          180) /
        Math.PI;
    }
  ...
  const translate = (event, element) => {
    ...
    deltaTranslation[0] =
      Math.round((translation[0] - translateStart[0]) / 5) * 5;
    deltaTranslation[1] =
      Math.round((translation[1] - translateStart[1]) / 5) * 5;

    // Callback to set the appropriate location attributes
    handleTranslate({
      x: originalTranslation[0] + deltaTranslation[0],
      y: originalTranslation[1] + deltaTranslation[1],
    });
  };
