...
const LocationSvg = (props) => {
    ...
    const iconSize = (() => {
        const { pixelsPerFoot, scaleUnit, iconSizeUnits } = settings
        if (!pixelsPerFoot || !iconSizeUnits || !d3?.actualDims || !d3?.naturalDims) return 20
        const displayScale = d3.actualDims.height / d3.naturalDims.height
        const sizeInFeet = scaleUnit === 'm' ? iconSizeUnits / 0.3048 : iconSizeUnits
        return sizeInFeet * pixelsPerFoot * displayScale
    })()
    useEffect(() => {
      setColour(color)
    ...
    return (
        ...
        >
            {((settings.mapApps?.labels || isSelected) && hoveringInfo?.id !== location?._id) &&
                <text y={-(iconSize / 2 + iconSize * 0.15)} fontSize={iconSize * 0.5} dominantBaseline="middle" textAnchor="middle">{location.name}</text>
            }
            <defs>
        ...
        {/*   <g
            ...
            */}
            ...
            >
                ...
                <svg id={`${rd3tClassName}-${schema}`} x={-iconSize / 2} y={-iconSize / 2} width={iconSize} height={iconSize} viewBox="0 0 400 400" style={{ filter: shouldGlow ? 'url(#glow2)' : 'none' }}>
