...
const LocationSvg = (props) => {
  ...
  return (
    <styled.WorkstationGroup
      ...
      transform={`translate(${location.x},${location.y}) rotate(${
        location.rotation
      }) scale(${d3.scale / d3.imgResolution})`}
    >
      <defs>
      ...
      >
        <circle
          ...
          strokeWidth="0"
          fill="transparent"
          style={{ cursor: rotating ? "pointer" : "grab" }}
        />
      ...
      <g
        ...
        transform={
          location.type === "device" && "scale(.07) translate(-180,-140)"
        }
      >
        <svg
          id={`${rd3tClassName}-${schema}`}
          x="-10"
          y="-10"
          width="20"
          height="20"
          viewBox="0 0 400 400"
          style={{ filter: shouldGlow ? "url(#glow2)" : "none" }}
        >
