...
function Station(props) {
  ...
  // If the process is broken, then you can only start the task at the route before break's unload location
    ...
    if (!routeStart || (routeStart && routeEnd)) {
      ...
      if (!isStationUnloadStation(routeBeforeBreak, station._id))
        disabled = true;
    }
    ...
    // setting unload
      ...
      if (isStationInRoutes(selectedProcess.routes, station._id))
        disabled = true;

      // always allow picking load station of route after the break, as this would fix the break
      const routeAfterBreak =
        selectedProcess.routes[selectedProcess.broken] || {};
    ...
    if (selectedProcess.routes.length > 0) {
      ...
      if (!routeStart || (routeStart && routeEnd)) {
        ...
        if (insertIndex === 0) {
          ...
          if (!isStationUnloadStation(previousRoute, station._id))
            disabled = true;
        }
      } else if (!routeEnd) {
  ...
  if (
    !!selectedTask &&
    (selectedTask.load.station === station._id ||
      selectedTask.unload.station === station._id)
  )
    highlight = true;
  ...
  const onSetStationTask = () => {
    ...
    if (
      selectedTask !== null &&
      (station.type === "human" || station.type === "warehouse")
    ) {
