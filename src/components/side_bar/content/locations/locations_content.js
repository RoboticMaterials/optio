...
export default function LocationContent() {
  ...
  const dispatchSetSelectedStation = (position) =>
    dispatch(setSelectedStation(position));
  ...
  return editingPosition || editingStation ? (
    <EditLocation />
  ) : (
    <ContentList
      title={"Locations"}
      ...
      elements={locationsSortedAlphabetically(Object.values(locations))
        // Filters out devices, entry positions, other positions and right click to move positions
        .filter(
          (location) =>
            !location.parent &&
            location.type !== "device" &&
            location.type !== "cart_entry_position" &&
            location.type !== "shelf_entry_position" &&
            location.type !== "charger_entry_position" &&
            location.type !== "other" &&
            location.schema !== "temporary_position" &&
            location.map_id === currentMap._id
        )}
