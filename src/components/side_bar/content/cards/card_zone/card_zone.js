...
const CardZone = (props) => {
  ...
  const cards =
    useSelector((state) => {
      return state.cardsReducer.processCards[processId];
    }) || [];
  const stations = useSelector((state) => {
    return state.stationsReducer.stations;
  });
  const draggedLotInfo = useSelector((state) => {
    return state.cardPageReducer.draggedLotInfo;
  });
  const { lotId: draggingLotId = "", binId: draggingBinId = "" } =
    draggedLotInfo || {};
  ...
  currentProcess.routes &&
    currentProcess.routes.forEach((currRouteId, index) => {
  ...
  Object.values(cards).forEach((card) => {
    ...
    const { bins, _id, ...rest } = card;

    const matchesFilter = card.name
      .toLowerCase()
