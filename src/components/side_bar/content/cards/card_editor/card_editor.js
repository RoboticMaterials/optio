...
const FormComponent = (props) => {
  ...
  const submitDisabled =
    (errorCount > 0 || touchedCount === 0 || isSubmitting) && submitCount > 0; // disable if there are errors or no touched field, and form has been submitted at least once
  ...
  const renderMoveContent = () => {
    ...
    const destinationOptions = [
      ...Object.values(stations).filter((currStation) => {
        if (currStation._id !== binId && processStationIds[currStation._id])
          return true;
      }),
    ];
    if (binId !== "QUEUE")
      destinationOptions.unshift({ name: "Queue", _id: "QUEUE" });
    if (binId !== "FINISH")
      destinationOptions.push({ name: "Finished", _id: "FINISH" });
      ...
      <styled.BodyContainer minHeight={"20rem"}>
        <div>
          ...
          >
            ...
            <NumberField maxValue={maxValue} minValue={0} name={"moveCount"} />
          </div>
    ...
    return (
      <styled.ProcessFieldContainer>
        ...
        <ScrollingButtonField
          ...
          valueKey={"value"}
          labelKey={"label"}
          options={processOptions.map((currProcessId, currIndex) => {
    ...
    return (
      <styled.StyledForm>
        ...
        <styled.Header>
          ...
          <styled.Title>
            {formMode === FORM_MODES.CREATE ? "Create Lot" : "Edit Lot"}
          </styled.Title>

          <Button secondary onClick={close} schema={"error"}>
  ...
  const handleSubmit = async (values, formMode) => {
    ...
    if (content === CONTENT.MOVE) {
      ...
      if (moveCount && moveLocation) {
        ...
        const { name: moveName, _id: destinationBinId } = moveLocation[0];
        ...
        const currentBinCount =
          parseInt(currentBin ? currentBin.count : 0) - moveCount;
        const destinationBinCount =
          parseInt(destinationBin ? destinationBin.count : 0) + moveCount;
