...
const TimelineZone = SortableContainer((props) => {
  const { handleCardClick, initialProcesses } = props;

  const processes = useSelector((state) => {
    return Object.values(state.processesReducer.processes);
  });
  const stations = useSelector((state) => {
    return state.locationsReducer.stations;
  });
  const cards = useSelector((state) => {
    return Object.values(state.cardsReducer.cards);
  });
    ...
    return (
      <div>
        {selectedProcesses.map((currProcess) => {
          ...
          const { _id: processId, name: processName } = currProcess;
            ...
            <styled.ProcessContainer>
              ...
              <styled.ProcessBody>
                {processCards.map((card, index) => {
                  ...
                  const { name: stationName } = cardStation;
  ...
  return (
    <styled.Container>
      ...
      />
      <styled.Body>{renderProcessCards()}</styled.Body>
    </styled.Container>
  );
});
