...
const ZoneContainer = (props) => {
  const { handleCardClick, currentProcess, id } = props;

  const isCardDragging = useSelector((state) => {
    return state.cardPageReducer.isCardDragging;
  });
  const isHoveringOverColumn = useSelector((state) => {
    return state.cardPageReducer.isHoveringOverColumn;
  });
  ...
  return (
    ...
    >
      ...
      >
        <TransformComponent>
          {{
            summary: <div>THIS WILL BE THE SUMMARY ZONE</div>,
            timeline: (
              <TimelineZone
