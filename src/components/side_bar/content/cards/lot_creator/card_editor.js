...
import {
  ...
  postLot,
  putLot,
} from "../../../../../redux/actions/lot_actions";
...
const CardEditor = (props) => {
  ...
  const stations = useSelector((state) => {
    return state.locationsReducer.stations;
  });
  ...
  const DispatchPutLot = async (lot, lotId) =>
    await dispatch(putLot(lot, lotId));
  const onGetLot = async (lotId) => await dispatch(getLot(lotId));

  const onGetCardHistory = async (cardId) =>
    await dispatch(getCardHistory(cardId));
  const onDeleteCard = async (cardId, processId) =>
      ...
      if (moveCount && moveLocation) {
        ...
        Object.values(cards).forEach((currCard, cardIndex) => {
          ...
          if (currCard.lot_id === card.lot_id) {
            ...
            if (
              currCard.route_id === moveRouteId &&
              currCard.station_id === moveStationId
            ) {
    ...
    >
        ...
        initialValues={{
          ...
          bin: card
            ? dropdownOptions.filter(
                (currOption) =>
                  currOption.station_id === card.station_id &&
                  currOption.route_id === card.route_id
              )
            : [dropdownOptions[0]],
      ...
      >
        {(formikProps) => {
          ...
          const startDateText =
            values?.dates?.start?.month &&
            values?.dates?.start?.day &&
            values?.dates?.start?.year
              ? values.dates.start.month +
                "/" +
                values.dates.start.day +
                "/" +
          ...
          const endDateText =
            values?.dates?.end?.month &&
            values?.dates?.end?.day &&
            values?.dates?.end?.year
              ? values.dates?.end.month +
                "/" +
                values.dates?.end.day +
                "/" +
          ...
          const touchedReducer = (accumulator, currentValue) =>
          ...
          const renderMove = () => {
            ...
            return (
              <styled.BodyContainer>
                ...
                <Button onClick={() => setContent(null)} schema={"processes"}>
                  Ok
                </Button>
              </styled.BodyContainer>
            );
              ...
              <styled.BodyContainer>
                ...
                <span>
                  <styled.DatesContainer>
                    <styled.DateItem
                      onClick={() => setContent(CONTENT.CALENDAR_START)}
                    >
                    ...
                    <styled.DateItem
                      onClick={() => setContent(CONTENT.CALENDAR_END)}
                    >
              ...
              <styled.BodyContainer>
                ...
                <styled.HistoryBodyContainer>
                  {events.map((currEvent) => {
                    ...
                    if (
                      Object.keys(modifiedData).includes("station_id") ||
                      Object.keys(modifiedData).includes("route_id")
                    ) {
                      // handle station_id change
                      if (Object.keys(modifiedData).includes("station_id")) {
                        const {
                          station_id: { new: newStationId, old: oldStationId },
                          ...rest
                        } = modifiedData;
                        ...
                        modifiedData = {
                          ...rest,
                          station: {
                            new: stations[newStationId]
                              ? stations[newStationId].name
                              : "",
                            old: stations[oldStationId]
                              ? stations[oldStationId].name
                              : "",
                          },
                        };
                      }
                      ...
                      if (Object.keys(modifiedData).includes("route_id")) {
                        const {
                          route_id: { new: newRouteId, old: oldRouteId },
                          ...rest
                        } = modifiedData;
                        ...
                        modifiedData = {
                          ...rest,
                          route: {
                            new: routes[newRouteId]
                              ? routes[newRouteId].name
                              : "",
                            old: routes[oldRouteId]
                              ? routes[oldRouteId].name
                              : "",
                          },
                        };
                      }
                    ...
                    let messages = parseMessageFromEvent(
                      name,
                      username,
                      modifiedData
                    );
          ...
          return (
            <styled.StyledForm>
              <styled.Header>
                {(content === CONTENT.CALENDAR_START ||
                  content === CONTENT.CALENDAR_END ||
                ...
                <styled.Title>
                  Card Editor
                  {lot && lot.name && <span>{lot.name}</span>}
                </styled.Title>

                <Button onClick={close} schema={"processes"}>
              ...
              {content === null && renderContent()}
              {(content === CONTENT.CALENDAR_END ||
                content === CONTENT.CALENDAR_START) &&
                renderCalendar()}
              {content === CONTENT.HISTORY && renderHistory()}
              {content === CONTENT.MOVE && renderMove()}
            </styled.StyledForm>
          );
        }}
      </Formik>
