...
const Column = (props) => {
  ...
  const dispatchSetCardDragging = async (lotId, binId) =>
    await dispatch(setCardDragging(lotId, binId));
    ...
    if (payload === null) {
      ...
      if (addedIndex !== null) {
        ...
        if (!(binId === station_id)) {
          ...
          const { [binId]: movedBin, ...remainingOldBins } = oldBins || {};
  ...
  const renderCards = () => {
    return (
      <styled.BodyContainer dragEnter={dragEnter}>
