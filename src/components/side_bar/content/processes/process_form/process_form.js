...
const ProcessForm = (props) => {
  const { toggleEditingProcess } = props;

  const dispatchSetSelectedTask = (task) => dispatch(setSelectedTask(task));

  const dispatch = useDispatch();
  const dispatchPostProcess = async (process) =>
    await dispatch(postProcesses(process));

  const dispatchPutProcess = async (process) =>
    await dispatch(putProcesses(process));

  const dispatchPostRouteClean = async (route) =>
    await dispatch(taskActions.postRouteClean(route));
  ...
  const dispatchDeleteProcess = async (ID) =>
    await dispatch(deleteProcesses(ID));
  const dispatchDeleteRouteClean = (routeId) =>
    dispatch(deleteRouteClean(routeId));
  const dispatchSaveFormRoute = async (formRoute) =>
    await dispatch(saveFormRoute(formRoute));
  ...
  const handleSave = async (values, close) => {
    // extract some values
    const { newRoute, changed, ...remainingValues } = values;
    ...
    const mappedRoutes = remainingValues.routes.map(
      (currRoute) => currRoute._id
    );
  ...
  const handleInitialRoutes = () => {
    if (
      ...
      Array.isArray(selectedProcess.routes)
    ) {
      let prevObj = null;
      ...
      return selectedProcess.routes
        .filter((currRouteItem) => {
          return isObject(
            isObject(currRouteItem) ? currRouteItem : tasks[currRouteItem]
          );
        })
        .map((currRouteItem) => {
          const route = isObject(currRouteItem)
            ? currRouteItem
            : tasks[currRouteItem];
  ...
  return (
    <Formik
      ...
      onSubmit={async (values, formikHelpers) => {
        const { setSubmitting, setTouched, resetForm } = formikHelpers;
