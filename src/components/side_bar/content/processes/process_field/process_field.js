...
export const ProcessField = (props) => {
  const { formikProps, onDelete, onSave, onBack } = props;
  ...
  const submitDisabled =
    errorCount > 0 || touchedCount === 0 || isSubmitting || !values.changed; //&& (submitCount > 0) // disable if there are errors or no touched field, and form has been submitted at least once

  const dispatch = useDispatch();
  const dispatchSetSelectedTask = async (task) =>
    await dispatch(setSelectedTask(task));
  const dispatchSetSelectedProcess = (process) =>
    dispatch(setSelectedProcess(process));
  const onHandlePostTaskQueue = (props) => dispatch(handlePostTaskQueue(props));
  const dispatchSetFixingProcess = async (bool) =>
    await dispatch(setFixingProcess(bool));
  const dispatchDeleteRouteClean = async (routeId) =>
  ...
  // throttled func
  const [dispatchSetSelectedProcess_Throttled] = useState(() =>
    throttle(() => {
  ...
  const handleAddTask = async () => {
    ...
    // not a new route
      ...
      const { value: currRouteValue } = fieldMeta;

      const routeProcesses = getRouteProcesses(currRouteValue._id); // get routes processes
      const belongsToAnotherProcess = routeProcesses.findIndex(
        (currProcess) => currProcess._id !== values._id
      ); // does route belong to another process?
  ...
  const updateExistingRoute = () => {
    ...
    const { needsSubmit, ...remainingValues } = currRouteValue || {};
  ...
  const cloneRoute = async () => {
    ...
    const fieldMeta = getFieldMeta(editingTask);
    const { initialValue, value: currRouteValue } = fieldMeta;

    const { changed, needsSubmit, ...remainingValues } = currRouteValue || {};
  ...
  const handleRemoveRoute = (routeId) => {
    ...
    const index = values.routes.findIndex(
  ...
  const handleDeleteRoute = async (routeId) => {
    ...
    setFieldValue(
      "routes",
      values.routes.filter((currRoute) => currRoute._id !== routeId)
    );
  };
  ...
  const renderRoutes = (routes) => {
    return routes
      ...
      .map((currRoute, currIndex) => {
        const { _id: currRouteId = "" } = currRoute || {};
            ...
            {!!values.broken && currIndex === values.broken - 1 && (
              <Button
                ...
                onClick={() => {
                  ...
                  const newTask = {
                    ...generateDefaultRoute(prevObj),
                    temp: { insertIndex: values.broken },
                  };
      ...
      <>
        <Button
          ...
          onClick={() => {
            ...
            const newTask = {
              ...generateDefaultRoute(prevObj),
              temp: { insertIndex: values.routes.length },
            };
            setFieldValue("newRoute", newTask);
            dispatchSetSelectedTask(newTask);
            setEditingTask("newRoute");
    ...
    <>
      ...
      {showExistingTaskWarning && (
        <ConfirmDeleteModal
          ...
          title={
            "Changing an existing route will affect other processes that use this route. Would you like to make a copy, or change the existing route?"
          }
      ...
      <styled.Container>
        ...
        {editingTask && selectedTask ? (
          ...
          <>
            ...
            <styled.SectionContainer showTopBorder={values.routes.length > 0}>
