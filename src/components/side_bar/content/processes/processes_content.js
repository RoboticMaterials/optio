...
const ProcessesContent = () => {
  ...
  useEffect(() => {
    ...
    Object.values(processes).map((process) => {
      const processRoutes =
        process.routes.map((currRoute) => tasks[currRoute]) || {};
      ...
      // Else if the process is broken, so fix that ish
        ...
        !!isBrokenProcess(processRoutes, tasks) &&
        process.broken === null
      ) {
  ...
  if (editing && selectedProcess !== null) {
    // Editing Mode
    return (
      <ProcessForm toggleEditingProcess={(props) => dispatchEditing(props)} />
    );
  } else {
    // List Mode
    return (
