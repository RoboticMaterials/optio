...
const LoadUnloadFields = (props) => {
  const { fieldParent, setFieldValue, values } = props;

  const dispatch = useDispatch();
  const dispatchSetSelectedTask = (task) => dispatch(setSelectedTask(task));
  const dispatchSetTaskAttributes = (id, attr) =>
    dispatch(setTaskAttributes(id, attr));
  ...
  if (
    (!!stations[selectedTask.load.position] &&
      stations[selectedTask.load.position].type === "human") ||
    (!!stations[selectedTask.unload.position] &&
      stations[selectedTask.unload.position].type === "human")
  ) {
    humanLocation = true;
  ...
  return (
    <>
      ...
      <styled.RowContainer style={{ marginTop: "2rem" }}>
        <styled.Header style={{ marginTop: "0rem", marginRight: ".5rem" }}>
          Load
        </styled.Header>
        ...
        {!humanLocation && (
          ...
          >
            <styled.HelpText style={{ fontSize: "1rem", marginRight: ".5rem" }}>
              TimeOut:{" "}
            </styled.HelpText>
            ...
            <TimePickerField
              ...
              name={
                fieldParent ? `${fieldParent}.load.timeout` : "load.timeout"
              }
              style={{
                flex: "0 0 7rem",
                display: "flex",
              ...
              allowEmpty={false}
              defaultOpenValue={
                selectedTask.load.timeout
                  ? moment().set({
                      minute: selectedTask.load.timeout.split(":")[0],
                      second: selectedTask.load.timeout.split(":")[1],
                    })
                  : moment().set({ minute: 1, second: 0 })
              }
              defaultValue={
                selectedTask.load.timeout
                  ? moment().set({
      ...
      <TextField
        name={
          fieldParent ? `${fieldParent}.load.instructions` : "load.instructions"
        }
        schema={"tasks"}
      ...
      {!isMiRTask(selectedTask) && (
        <styled.ContentContainer style={{ paddingBottom: "0rem" }}>
          ...
          <styled.HelpText>
            Do you want to track transit time? This will display a Unload Button
      ...
      {!values.handoff && (
        <>
          ...
          <TextField
            name={
              fieldParent
                ? `${fieldParent}.unload.instructions`
                : "unload.instructions"
            }
            schema={"tasks"}
            focus={!!selectedTask && selectedTask.type == null}
          ...
          {!isOnlyHumanTask(selectedTask) && (
            ...
            >
              ...
              <DropDownSearchField
                name={
                  fieldParent ? `${fieldParent}.unload.sound` : "unload.sound"
                }
                placeholder="Select Sound"
                label="Sound to be played upon arrival"
                labelField="name"
                ...
                closeOnSelect="true"
                mapInput={(val) => {
                  if (isString(val) && isObject(sounds[val]))
                    return [sounds[val]];
                  return [];
                }}
                mapOutput={(val) => {
