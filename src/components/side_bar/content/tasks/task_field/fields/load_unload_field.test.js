...
mockStore.dispatch({
  ...
  payload: {
    task: {
      ...
      processes: ["2e0104ee-b145-4e88-9c84-6166e641284d"],
      load: {
...
test("Renders Load Unload Fields Correctly", () => {
  const { getByText, getByLabelText } = TestProviderRender(
    <LoadUnloadFields />,
    { store: mockStore }
  );
  ...
  getByText(
    "Do you want to track transit time? This will display a Unload Button at the Unload Station"
  );
});
