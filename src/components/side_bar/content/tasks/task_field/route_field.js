...
const TaskField = (props) => {
  ...
  const { value: values = {}, error: errors = {}, touched } = fieldMeta || {};

  // sets values.changed to true when a change occurs
  useChange(fieldParent);

  const { name, obj, track_quantity, _id: routeId, changed, temp } =
    values || {};

  const { insertIndex } = temp || {};
  ...
  const dispatchPutDashboard = (dashboard, ID) =>
    dispatch(putDashboard(dashboard, ID));
  const dispatchPostDashboard = (dashboard) =>
    dispatch(postDashboard(dashboard));
  const dispatchSetFixingProcess = (bool) => dispatch(setFixingProcess(bool));
  const dispatchSetSelectedStation = (station) =>
    dispatch(setSelectedStation(station));
  const dispatchSetSelectedPosition = (position) =>
    dispatch(setSelectedPosition(position));
  const dispatchSetEditing = async (props) =>
    await dispatch(taskActions.editingTask(props));
  ...
  useEffect(() => {
    ...
    if (selectedTask && selectedTask.load) {
      setFieldValue(
        fieldParent ? `${fieldParent}.load.station` : "load.station",
        selectedTask.load.station,
        false
      );
      setFieldValue(
        fieldParent ? `${fieldParent}.load.position` : "load.position",
        selectedTask.load.position,
        false
      );
    }
    if (selectedTask && selectedTask.unload) {
      setFieldValue(
        fieldParent ? `${fieldParent}.unload.station` : "unload.station",
        selectedTask.unload.station,
        false
      );
      setFieldValue(
        fieldParent ? `${fieldParent}.unload.position` : "unload.position",
        selectedTask.unload.position,
        false
      );
    }
    ...
    if (selectedTask && selectedTask.type) {
      setFieldValue(
        fieldParent ? `${fieldParent}.type` : "type",
        selectedTask.type,
        false
      );
    }
    ...
    if (selectedTask && selectedTask.device_types) {
      setFieldValue(
        fieldParent ? `${fieldParent}.device_types` : "device_types",
        selectedTask.device_types,
        false
      );
    }
    ...
    if (isMiRTask(selectedTask)) {
      if (values.handoff)
        setFieldValue(
          fieldParent ? `${fieldParent}.handoff` : "handoff",
          false
        );
    ...
    if (name === prevName || !name) {
      setFieldValue(
        fieldParent ? `${fieldParent}.name` : "name",
        newName,
        false
      );
    }
    ...
    if (selectedTask.load.position === null) {
      ...
      return (
        <styled.DirectionText>
          Click a position on the map to be the load (or start) position.
        </styled.DirectionText>
      );
      ...
      return (
        <>
          {selectedTask.unload.position === null && (
            <styled.DirectionText>
              Click on a position on the map to be the end position.
            </styled.DirectionText>
          )}
          <styled.HelpText>
            Since the start position is not tied to a station, this task is no
            longer a transport task
          </styled.HelpText>
        </>
      );
    } else {
      ...
      if (selectedTask.unload.position === null) {
        ...
        return (
          <styled.DirectionText>
            Click on a position on the map to be the unload (or end) position.
          </styled.DirectionText>
        );
        ...
        return (
          <styled.HelpText>
            Since the end position is not tied to a station, this task is no
            longer a transport task
          </styled.HelpText>
        );
      } else {
  ...
  const createObject = async () => {
    ...
    if (isObject(obj) && "name" in obj) {
      if (obj._id == undefined) {
        // If the object does not currently exist, make a new one
        const newObject = {
          name: obj.name,
          description: "",
          modelName: "",
        ...
        //  Otherwise just set the task obj to the existing obj
        objectId = obj._id;
      }
    }

    setFieldValue(
      fieldParent ? `${fieldParent}.needsSubmit` : "needsSubmit",
      true
    );
  ...
  const updateDashboard = () => {
    ...
    if (updatedDashboard === undefined) {
      ...
      postDashboardPromise.then(async (postedDashboard) => {
        alert(
          "Added dashboard to location. There already should be a dashboard tied to this location, so this is an temp fix"
        );
      ...
      {!!selectedTask && (
        <styled.ContentContainer>
          ...
          {!!selectedTask && isProcessTask && !!selectedTask.new && (
            <>
              <styled.Label>
                <styled.LabelHighlight>Either</styled.LabelHighlight> choose an
                existing Route...
              </styled.Label>
              <DropDownSearch
                ...
                labelField="name"
                valueField="name"
                options={Object.values(routes).filter((task) => {
                  ...
                  if (fixingProcess) {
                    ...
                    const routeBeforeBreak =
                      selectedProcess.routes[selectedProcess.broken - 1];
                ...
                closeOnSelect="true"
                name={
                  fieldParent ? `${fieldParent}.existingRoute` : "existingRoute"
                }
                onChange={async (dropdownValues) => {
                  const selectedValue = dropdownValues[0] || {};

                  const { obj: selectedObjId = "", _id: selectedRouteId = "" } =
                    selectedValue || {};

                  const selectedObj = selectedObjId
                    ? objects[selectedObjId] || null
                    : null;
          ...
          {isTransportTask && (
            <>
              <TextboxSearchField
                ...
                onChange={(val) => {}}
                valueField="name"
                options={Object.values(objects).filter(
                  (obj) => obj.map_id === currentMap._id
                )}
                defaultValue={obj}
                textboxGap={0}
                closeOnSelect="true"
              ...
              <styled.HelpText>
                Select the object that will be transported. Either search &
                select an existing object, or type the name of a new object to
                create one.
              </styled.HelpText>
            ...
            <>
              {selectedProcess && (
                <Button
                  schema={"tasks"}
                  disabled={
                    !!selectedTask && !!selectedTask._id && !!selectedTask.new
                  }
                  primary
                  onClick={() => {
                    onRemove(routeId);
