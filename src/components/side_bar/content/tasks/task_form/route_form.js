...
const TaskForm = (props) => {
  const { initialValues, toggleEditing, isNew, ...remainingProps } = props;
  ...
  const dispatchRemoveTask = async (taskId) =>
    await dispatch(taskActions.removeTask(taskId));
  const dispatchDeleteRouteClean = async (routeId) =>
    await dispatch(taskActions.deleteRouteClean(routeId));
  const onEditing = async (props) =>
  ...
  return (
    <Formik
      ...
      onSubmit={async (values, formikHelpers) => {
        const { setSubmitting, setTouched } = formikHelpers;
