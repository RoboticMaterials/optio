...
export default function TaskContent(props) {
  ...
  const onTaskQueueItemClicked = (id) =>
    dispatch({ type: "TASK_QUEUE_ITEM_CLICKED", payload: id });
  const onHandlePostTaskQueue = (props) =>
    dispatch(taskQueueActions.handlePostTaskQueue(props));
  const onEditing = async (props) =>
    await dispatch(taskActions.editingTask(props));
  const dispatchSetSelectedTask = async (task) =>
    await dispatch(taskActions.setSelectedTask(task));
  const dispatchAddTask = async (task) =>
    await dispatch(taskActions.addTask(task));
    ...
    return (
      <>
        ...
        <ContentList
          title={"Routes"}
          schema={"tasks"}
          elements={tasksSortedAlphabetically(Object.values(tasks))
