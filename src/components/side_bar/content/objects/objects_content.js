...
export default function ObjectsContent(props) {
  ...
  let selectedObject = useSelector(
    (state) => state.objectsReducer.selectedObject
  );
  ...
  const updateType = (type) => {
    dispatch(
      objectActions.setObjectAttributes(selectedObject._id.$oid, { type })
    );
  };
  ...
  if (editing) {
    return (
      <styled.ContentContainer>
          ...
          <ContentHeader
            ...
            onClickSave={() => {
              if (selectedObject._id.$oid == "__NEW_OBJECT") {
                ...
                dispatch(
                  objectActions.putObject(
                    selectedObject,
                    selectedObject._id.$oid
                  )
                );
              }
              dispatch(objectActions.deselectObject());
        ...
        <Textbox
          ...
          onChange={(e) => {
            dispatch(
              objectActions.setObjectAttributes({ name: e.target.value })
            );
          }}
    ...
    return (
      <ContentList
        title={"Objects"}
        schema={"objects"}
        elements={Object.values(objects).filter(
          (item) => item.map_id === currentMap._id
        )}
        onMouseEnter={(object) =>
          dispatch(objectActions.selectObject(object._id.$oid))
        }
