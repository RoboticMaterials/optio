...
const CreateScheduleForm = (props) => {
  ...
  const tasksArr = Object.values(tasks).filter(
    (task) => task.map_id === currentMap._id
  ); // get copy of tasks as arr here instead of calling Object.values() multiple times
    ...
    if (selectedScheduleItem) {
      ...
      const initialValues = {
        ...
        start_time: selectedScheduleItem.start_time
          ? moment(timeString24HrToDate(selectedScheduleItem.start_time))
          : nowTimeString,
        schedule_on: selectedScheduleItem.schedule_on,
        time_interval: selectedScheduleItem.time_interval
          ? moment(timeString24HrToDate(selectedScheduleItem.time_interval))
          : nowTimeString,
        interval_on: selectedScheduleItem.interval_on,
        next_time: selectedScheduleItem.next_time,
        map_id: currentMap._id,
        stop_time: selectedScheduleItem.stop_time
          ? moment(timeString24HrToDate(selectedScheduleItem.stop_time))
          : nowTimeString,
        name: selectedScheduleItem.name ? selectedScheduleItem.name : "",
        selectedScheduleId: selectedScheduleItem.id,
        task:
          selectedScheduleItem.task_id && tasks[selectedScheduleItem.task_id]
