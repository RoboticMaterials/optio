...
const ScheduleList = (props) => {
  ...
  let [timeToNextScheduledTemp, setTimeToNextScheduledTemp] = useState(
    "10000000"
  );
  let [nextScheduledTimeTemp, setNextScheduledTimeTemp] = useState("10000000");

  let [timeToNextScheduledHours, setTimeToNextScheduledHours] = useState(
    "10000000"
  );
  let [timeToNextScheduledMinutes, setTimeToNextScheduledMinutes] = useState(
    "10000000"
  );
   ...
   * */
  const renderTasks = () => {
    let fullSchedulesArr = Object.values(schedules).filter(
      (item) => item.map_id === currentMap._id
    );
    return (
          ...
          fullSchedulesArr.map((item, index, arr) => {
            ...
            return (
              <ScheduleListItem
                ...
                days_on={daysOnArr}
                taskName={
                  selectedTask && selectedTask.name ? selectedTask.name : ""
                }
                ...
                disabled={
                  taskIsDeleted ||
                  item.task_id === "TEMP_NEW_SCHEDULE_ID" ||
                  item.task_id === "DEFAULT_TASK_ID"
                }
                error={error}
                next_time={next_time}
              />
  ...
  const handleNextExecution = () => {
    ...
    let currentTime =
      Number(moment(moment(), "HH:mm:ss").format("HH") * 60) +
      Number(moment(moment(), "HH:mm:ss").format("mm"));
    logger.log("nexttime");

    if (fullSchedulesArr.length > 0) {
      fullSchedulesArr.map((item, index, arr) => {
        var startTime =
          Number(moment(item.start_time, "HH:mm:ss").format("HH") * 60) +
          Number(moment(item.start_time, "HH:mm:ss").format("mm"));
        if (item.interval_on) {
          var stopTime =
            Number(moment(item.stop_time, "HH:mm:ss").format("HH") * 60) +
            Number(moment(item.stop_time, "HH:mm:ss").format("mm"));

          if (stopTime - currentTime > 0 && startTime - currentTime < 0) {
            var intervalTime =
              Number(moment(item.time_interval, "HH:mm:ss").format("HH") * 60) +
              Number(moment(item.time_interval, "HH:mm:ss").format("mm"));
            timeToNextScheduledTemp =
              Math.ceil((currentTime - startTime) / intervalTime) *
                intervalTime +
              startTime -
              currentTime;
            nextScheduledTimeTemp =
              Math.ceil((currentTime - startTime) / intervalTime) *
                intervalTime +
              startTime;
          }
  ...
  return (
    <styled.Container>
      ...
      <ContentHeader
        content={"scheduler"}
        onClickAdd={() => setShowScheduleCreator(true)}
      />
      {handleNextExecution()}
      {timeToNextScheduled < 1440 ? (
        ...
        >
          Next Up: " {nextScheduleName} " will execute in {timeToNextScheduled}{" "}
          minutes
        </styled.NextExecution>
      ) : (
        <></>
