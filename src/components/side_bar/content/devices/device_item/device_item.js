...
const DeviceItem = (props) => {
  const { device, isSmall, ind, setSelectedDevice, tasks, taskQueue } = props;
  ...
  if (DeviceTypes[device.device_model])
    deviceType = DeviceTypes[device.device_model];
  else if (device.device_model === "MiR100") deviceType = DeviceTypes["cart"];

  // Sets the location of the device
  useEffect(() => {
    if (!!device.station_id && device.device_model !== "MiR100" && !stationId)
  ...
  const handleDeviceStatus = () => {
    ...
    try {
      if (device.state_text === "EmergencyStop") {
        ...
        try {
          deviceStatus =
            taskQueue[device.current_task_queue_id.$oid].mission_status;
        } catch (error) {
          deviceStatus = "No Active Task";
        }
      } else {
  ...
  const handleDeviceBattery = () => {
    ...
    return (
      <>
        <styled.BatteryText
          isSmall={isSmall}
          style={{ color: batteryTextColor }}
        >
          {deviceBattery.toFixed(0)}%
        </styled.BatteryText>

        {/* SVG for the battery percent dots. Uses the above arrays to determine what percentage to show */}
        <styled.BatterySvg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 67.6 218.09"
        >
          <g id="Layer_2" data-name="Layer 2">
            <g id="Layer_1-2" data-name="Layer 1">
              <rect
                fill={batteryRectFill[4]}
                stroke={batteryRectStroke[4]}
                strokeWidth=".3rem"
                x="13.86"
                y="174.04"
                width={batteryRectWidth}
                height={batteryRectWidth}
                rx={batteryRectRx}
                transform="translate(-169.27 153.88) rotate(-69.46)"
              />
              <rect
                fill={batteryRectFill[3]}
                stroke={batteryRectStroke[3]}
                strokeWidth=".3rem"
                x="25.91"
                y="130.42"
                width={batteryRectWidth}
                height={batteryRectWidth}
                rx={batteryRectRx}
                transform="translate(-121.38 165.29) rotate(-79.65)"
              />
              <rect
                fill={batteryRectFill[2]}
                stroke={batteryRectStroke[2]}
                strokeWidth=".3rem"
                x="17.6"
                y="96.54"
                width={batteryRectWidth}
              ...
              />
              <rect
                fill={batteryRectFill[1]}
                stroke={batteryRectStroke[1]}
                strokeWidth=".3rem"
                x="13.41"
                y="50.17"
                width={batteryRectWidth}
                height={batteryRectWidth}
                rx={batteryRectRx}
                transform="translate(-10.63 7.92) rotate(-10.35)"
              />
              <rect
                fill={batteryRectFill[0]}
                stroke={batteryRectStroke[0]}
                strokeWidth=".3rem"
                x="1.36"
                y="6.54"
                width={batteryRectWidth}
                height={batteryRectWidth}
                rx={batteryRectRx}
                transform="translate(-5.01 10.46) rotate(-20.54)"
              />
            </g>
          </g>
        </styled.BatterySvg>
  ...
  return (
    <styled.DeviceContainer
      ...
      onMouseLeave={() => {
        !!stationId && dispatchSetSelectedStation(null);
      }}
    >
      <styled.BigCircle isSmall={isSmall}>
        ...
        <styled.DeviceIcon
          ...
          style={{
            fontSize: !!deviceType.deviceFontSize && deviceType.deviceFontSize,
            color:
              !!stationId || deviceType.icon === "icon-cart"
                ? deviceType.primaryColor
                : "white",
          }}
        />
      ...
      {!!device.battery_percentage && handleDeviceBattery()}
