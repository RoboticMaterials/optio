...
const DeviceEdit = (props) => {
  const { deviceLocationDelete } = props;
  ...
  const dispatchAddStation = (selectedStation) =>
    dispatch(addStation(selectedStation));
  const dispatchSetSelectedStation = (selectedStation) =>
    dispatch(setSelectedStation(selectedStation));
  const onSetSelectedDevice = (selectedDevice) =>
    dispatch(deviceActions.setSelectedDevice(selectedDevice));
  const onAddPosition = (updatedPosition) =>
    dispatch(positionActions.addPosition(updatedPosition));

  const selectedDevice = useSelector(
    (state) => state.devicesReducer.selectedDevice
  );
  ...
  const handleDeviceMapLocation = () => {
    ...
    if (DeviceItemTypes[selectedDevice.device_model])
      deviceType = DeviceItemTypes[selectedDevice.device_model];
    else if (selectedDevice.device_model === "MiR100")
    ...
    return (
      ...
      >
        <styled.ConnectionText>
          To add the device to the screen, grab the device with your mouse and
    ...
    return (
      <styled.SectionsContainer>
        ...
        <DropDownSearch
          ...
          values={
            selectedDevice.idle_location
              ? [positions[selectedDevice.idle_location]]
              : []
          }
          dropdownGap={5}
