...
const App = (props) => {
  ...
  const getFailureCount = useSelector(
    (state) => state.taskQueueReducer.getFailureCount
  );
  ...
  const authenticated = useSelector(
    (state) => state.localReducer.localSettings.authenticated
  );
  ...
  const handleMobileMapView = () => {
    if (!widgetPageLoaded) {
      return (
        <Route path={["/locations/:stationID?/:widgetPage?", "/"]}>
    ...
    <>
      ...
      <ThemeProvider theme={theme[stateTheme]}>
        <styled.Container>
          <ConfirmDeleteModal
            isOpen={
              getFailureCount < 10 || showStopAPIModal === false ? false : true
            }
            title={
              "Oops! It looks like the server is diconnected. Would you like to turn off updates from the backend?"
            }
            button_1_text={"Yes"}
            handleOnClick1={() => {
          ...
          <BrowserRouter>
            ...
            <Route
              path={[
                "/locations/:stationID?/:widgetPage?",
                "/:sidebar?/:data1?/:data2?",
                "/",
              ]}
            >
              {authenticated && (
                <ApiContainer
                  ...
                  apiLoaded={() => setApiLoaded(true)}
                  isApiLoaded={apiLoaded}
                />
              )}
            </Route>
            ...
            {loaded && authenticated && apiLoaded && (
              <styled.ContentContainer>
                ...
                <styled.BodyContainer>
                  ...
                  <Route
                    path={[
                      "/locations/:stationID/dashboards/:dashboardID?",
                      "/",
                    ]}
                    component={HILModal}
                  />
