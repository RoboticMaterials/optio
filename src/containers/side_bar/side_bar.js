...
const SideBar = (props) => {
  ...
  const dispatchSetSelectedPosition = (station) =>
    dispatch(setSelectedPosition(station));
  ...
  const processEditing = useSelector(
  ...
  useEffect(() => {
    ...
    const time = Date.now();
    if (
      ((page === "processes" || page === "lots") && subpage === "lots") ||
      id === "timeline" ||
      id === "summary"
    ) {
      ...
      (prevSubpage === "lots" ||
        prevId === "timeline" ||
        prevId === "summary") &&
      (prevPage === "processes" || prevPage === "lots") &&
      (subpage !== "lots" || id === "timeline" || id === "summary")
    ) {
      setWidth(prevWidth);
    ...
    <>
      ...
      {showSideBar && (
        <styled.SidebarWrapper
          mode={mode}
          style={{ width: showSideBar == true ? width : 0, display: "flex" }}
          open={showSideBar}
        >
          <SideBarSwitcher
          ...
          <styled.SidebarContent key="sidebar-content" style={{}}>
            {content}
