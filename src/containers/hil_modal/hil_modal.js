...
const HILModal = () => {
  ...
  const onSetHilTimers = (timers) =>
    dispatch({ type: "HIL_TIMERS", payload: timers });
  const onSetActiveHilDashboards = (active) =>
  ...
  const localHumanTask = useSelector(
    (state) => state.taskQueueReducer.localHumanTask
  );
  ...
  const handleHilsInTaskQueue = useMemo(() => {
    ...
    if (
      ...
      taskQueue[taskQueueItemClicked] &&
      !!taskQueue[taskQueueItemClicked].hil_station_id
    ) {
      ...
      return (
        <HILModals
          hilMessage={item.hil_message}
          hilType={hilType}
          taskQuantity={item.quantity}
          taskQueueID={taskQueueItemClicked}
          item={item}
        />
      );
    } else {
    ...
    return Object.values(taskQueue).map((item) => {
      ...
      // The HIL modal needs to immediatly show because the backend will be too slow to respond to show that dashboard after button click
        ...
        !!item.dashboard &&
        item.dashboard === dashboardID &&
        localHumanTask === item._id
      ) {
        let hilMessage = item.hil_message;
        if (!hilMessage) {
          hilMessage = tasks[item.task_id].load.instructions;
