...
const StatusHeader = (props) => {
  ...
  const taskQueueOpen = useSelector(
    (state) => state.taskQueueReducer.taskQueueOpen
  );
  ...
  const onTaskQueueOpen = (props) =>
    dispatch(taskQueueActions.taskQueueOpen(props));
  ...
  const generatePath = () => {
    ...
    } else if (!taskQueueOpen && sideBarWidth + 120 > pageWidth - 320) {
      // No notifications, overlaping containers
      x = (pageWidth - sideBarWidth - 200) / 2;
      mergeHeight = 5 + ((sideBarWidth - pageWidth + 440) * 35) / 120;
    } else if (taskQueueOpen && sideBarWidth + 120 > pageWidth - 440) {
      // W/ notifications, overlapping containers
    ...
    const path = `
                ...
                C${
                  isSideBarOpen && !widgetPage
                    ? sideBarWidth + x / 2
                    : leftMargin + x / 2
                },40 ${
      isSideBarOpen && !widgetPage ? sideBarWidth + x / 2 : leftMargin + x / 2
    },${mergeHeight} ${
      isSideBarOpen && !widgetPage ? sideBarWidth + x : leftMargin + x
    },${mergeHeight}
                L${
                  pageWidth -
                  (taskQueueOpen ? rightCurvePoint : rightMargin) -
                  x
                },${mergeHeight}
                C${
                  pageWidth -
      ...
      taskQueueOpen
        ? `${pageWidth - rightCurvePoint},${mergeHeight}`
        : `${pageWidth - rightMargin - x / 2},40`
    } ${pageWidth - (taskQueueOpen ? rightCurvePoint : rightMargin)},40
                L${pageWidth},40
                L${pageWidth},0
                Z
            `;
      ...
      <styled.RightContentContainer>
        ...
        {windowWidth > widthBreakPoint && (
          <>
            <styled.RightMenuContainer
              checked={taskQueueOpen}
              onClick={() => onTaskQueueOpen(!taskQueueOpen)}
            >
              {toggle === "notifications" ? (
                <>
                  <styled.NotificationText>2</styled.NotificationText>
