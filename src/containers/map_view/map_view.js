...
export class MapView extends Component {
  ...
  componentDidMount() {
    ...
    window.addEventListener(
      ...
      () => {
        this.mouseDown = false;
        this.validateNewEntity();
      },
      { passive: false }
    );
    window.addEventListener("click", () => {
      this.setState({ showRightClickMenu: {} });
    });
  ...
  checkForMapLoad = () => {
    var defaultMap = this.props.maps.find(
      (map) => map._id === this.props.currentMapId
    );

    if (
      this.props.currentMapId &&
      this.props.currentMap &&
      this.props.currentMap._id &&
      defaultMap
    ) {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (widgetPage) {
      ...
      document.addEventListener("dragend", this.validateNewLocation, {
        capture: false,
        passive: true,
      });
    }
  ...
  dragNewEntity = (e) => {
    ...
    if (
      ...
      this.props.selectedStation.temp === true
    ) {
    ...
    // Handle Positions
      ...
      this.props.selectedPosition.temp === true &&
      this.props.selectedPosition.schema !== "temporary_position"
    ) {
      this.props.dispatchSetPositionAttributes(
        this.props.selectedPosition._id,
        {
          x: e.clientX,
          y: e.clientY,
        }
    ...
    // Else it's a stations child position
      ...
      const draggingChild = Object.values(
        this.props.selectedStationChildrenCopy
      ).find((position) => position.temp === true);
  ...
  validateNewEntity = () => {
    ...
    if (
      ...
      const pos = convertD3ToReal(
        [this.props.selectedStation.x, this.props.selectedStation.y],
        this.d3
      );
      this.props.dispatchSetStationAttributes(this.props.selectedStation._id, {
        pos_x: pos[0],
    ...
    // Handle Posiitions
      ...
      const pos = convertD3ToReal(
        [this.props.selectedPosition.x, this.props.selectedPosition.y],
        this.d3
      );
      this.props.dispatchSetPositionAttributes(
        this.props.selectedPosition._id,
        ...
        .on("zoom", () => {
          ...
          if (
            (!!this.props.selectedStation ||
              (!!this.props.selectedPosition &&
                this.props.selectedPosition.schema !== "temporary_position")) &&
            (!this.props.editingStation || !this.props.editingPosition)
          ) {
          ...
          if (this.props.selectedStation) {
            [x, y] = convertRealToD3(
              [
                this.props.selectedStation.pos_x,
                this.props.selectedStation.pos_y,
              ],
              this.d3
            );
          ...
          this.props.dispatchUpdateStations(
            stations,
            updatedSelectedStation,
            this.d3
          ); // Bulk Update
          ...
          if (this.props.selectedPosition) {
            [x, y] = convertRealToD3(
              [
                this.props.selectedPosition.pos_x,
                this.props.selectedPosition.pos_y,
              ],
              this.d3
            );
          ...
          if (this.props.selectedStationChildrenCopy) {
            ...
            Object.values(this.props.selectedStationChildrenCopy).forEach(
              (position) => {
                [x, y] = convertRealToD3(
          ...
          this.props.dispatchUpdatePositions(
            positions,
            updatedSelectedPosition,
            updatedChildrenPositions,
            this.d3
          ...
          Object.values(devices)
            .filter((device) => device.device_model == "MiR100")
            .map((device) => {
              [x, y] = convertRealToD3(
                [device.position.pos_x, device.position.pos_y],
                this.d3
              );
  ...
  render() {
    ...
    return (
      <div
        ...
        onMouseMove={this.dragNewEntity}
        onMouseUp={this.validateNewLocation}
      >
        <styled.MapContainer
          ref={(mc) => (this.mapContainer = mc)}
          style={{ pointerEvents: this.widgetDraggable ? "default" : "none" }}
        >
          ...
          {Object.keys(this.state.showRightClickMenu).length > 0 && (
            <RightClickMenu
              coords={this.state.showRightClickMenu}
              buttonClicked={() => {
                this.setState({ showRightClickMenu: {} });
              }}
              d3={this.d3}
            ...
            onMouseEnter={() => {
              if (this.props.widgetLoaded) {
                ...
                if (
                  (!!this.props.selectedStation ||
                    (!!this.props.selectedPosition &&
                      this.props.selectedPosition.schema !==
                        "temporary_position")) &&
                  (!this.props.editingStation || !this.props.editingPosition)
                ) {
                  this.props.dispatchHoverStationInfo(null);
            ...
            onMouseOver={() => {
              if (this.props.widgetLoaded) {
                ...
                if (
                  ...
                  if (
                    !this.props.editingStation ||
                    !this.props.editingPosition
                  ) {
                    this.props.dispatchSetSelectedStation(null);
                    this.props.dispatchSetSelectedPosition(null);
                  }
          ...
          >
            ...
            <styled.MapGroup className={this.rd3tMapClassName}>
              {/* Foreign object allows an image to be put in the SVG container */}
              <foreignObject width="200%" height="200%">
                {!!this.props.currentMap && (
                  <styled.MapImage
                    ref={(mi) => (this.mapImage = mi)}
                    tall={
                      !!this.mapContainer && // Fixes the map sizing - cutoff issue
                      this.mapContainer.getBoundingClientRect().height /
                        this.naturalImageDimensions.height >
                        this.mapContainer.getBoundingClientRect().width /
                          this.naturalImageDimensions.width
                    }
            ...
            {!!this.props.selectedTask && <TaskPaths d3={this.d3} />}
            {!!this.props.selectedProcess && <ProcessPaths d3={this.d3} />}
            <defs>
                ...
                <>
                  {
                    ...
                    Object.values(stations)
                      .filter(
                        (station) =>
                          station.map_id === this.props.currentMap._id
                      )
                      .map((station, ind) => (
                        <Station
                          key={`loc-${ind}`}
                ...
                <>
                  {
                    ...
                    Object.values(positions)
                      .filter(
                        (position) =>
                          position.map_id === this.props.currentMap._id
                      )
                      .map((position, ind) => (
                        <Position
                          key={`pos-${ind}`}
                          position={
                            !!selectedPosition &&
                            position._id === selectedPosition._id
                              ? // If there is a selected station, then render the selected station vs station in redux
                                // Selected station could contain local edits that are not on the backend (naked redux) yet
                                selectedPosition
                              : // If the positions parent is currently being edited
                              !!selectedStationChildrenCopy &&
                                position._id in selectedStationChildrenCopy
          ...
          {this.props.hoveringInfo !== null && !this.mobileMode && <Widgets />}
        </styled.MapContainer>
      </div>
    );
  }
...
const mapStateToProps = function (state) {
  return {
    ...
    selectedStationChildrenCopy:
      state.positionsReducer.selectedStationChildrenCopy,
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    dispatchUpdateStations: (stations, selectedStation, d3) =>
      dispatch(updateStations(stations, selectedStation, d3)),
    dispatchUpdatePositions: (
      ...
      dispatch(
        updatePositions(positions, selectedPosition, childrenPositions, d3)
      ),
    dispatchUpdateDevices: (devices, d3) =>
      dispatch(deviceActions.updateDevices(devices, d3)),

    dispatchPostPosition: (position) => dispatch(postPosition(position)),
    dispatchSetStationAttributes: (id, attr) =>
      dispatch(setStationAttributes(id, attr)),
    dispatchSetPositionAttributes: (id, attr) =>
