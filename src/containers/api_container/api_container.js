...
const ApiContainer = (props) => {
  ...
  const onPostLocalSettings = (settings) =>
  ...
  const onPutDevice = async (device, ID) =>
    await dispatch(putDevices(device, ID));
  const onPutPosition = (position, ID) => dispatch(putPosition(position, ID));
  const onPutProcess = (process) => dispatch(putProcesses(process));
  const onPutStation = async (station, ID) =>
    await dispatch(putStation(station, ID));
  ...
  const dispatchStopAPICalls = (bool) =>
    dispatch(localActions.stopAPICalls(bool));

  // Selectors
  const schedulerReducer = useSelector((state) => state.schedulerReducer);
  const devices = Object.values(
    useSelector((state) => {
      return state.devicesReducer;
    })?.devices || {}
  );
  ...
  const mapViewEnabled = useSelector(
    (state) => state.localReducer.localSettings.mapViewEnabled
  );
  ...
  useEffect(() => {
    ...
    if (!MiRMapEnabled) {
      ...
      Object.values(devices).forEach((currDevice, index) => {
        const device_model = currDevice?.device_model
          ? currDevice?.device_model
          : "";
        // const x = currDevice?.position?.x
        // const y = currDevice?.position?.y
        if (device_model === "MiR100") containsMirCart = true;
      });
    ...
    Object.values(devices).map(async (device) => {
      ...
      device.dashboards.map((dashboard) => {
        if (!dashboards[dashboard]) {
          console.log(
            "QQQQ Dashboard has dissapeared for some reason",
            dashboard
          );
  ...
  const handleTasksWithBrokenPositions = async (tasks, stations, positions) => {
    ...
    Object.values(tasks).map(async (task) => {
      ...
      if (
        (!positions[task.load.position] && !stations[task.load.position]) ||
        (!positions[task.unload.position] && !stations[task.unload.position])
      ) {
        console.log(
          "QQQQ Position doesnt exist in positions, DELETE TASK",
          task._id
        );
      ...
      if (
        (!!positions[task.load.position] &&
          !!positions[task.load.position].change_key &&
          positions[task.load.position].change_key === "deleted") ||
        (!!positions[task.unload.position] &&
          !!positions[task.unload.position].change_key &&
          positions[task.unload.position].change_key === "deleted")
      ) {
        console.log(
          "QQQQ Position is deleted, waiting on back end, DELETE TASK"
        );
  ...
  const handlePositionsWithBrokenParents = async (stations, positions) => {
    ...
    Object.values(positions).map(async (position) => {
      if (
        !!position.parent &&
        !Object.keys(stations).includes(
          position.parent && position.change_key !== "deleted"
        )
      ) {
    ...
    Object.values(stations).map((station) => {
      ...
      station.children.map(async (position, ind) => {
        if (!!positions[position] && positions[position].parent === null) {
          ...
          console.log(
            "QQQQ Stations with deleted position",
            deepCopy(brokenStation),
            deepCopy(positions)
          );
  ...
  const handleDevicesWithBrokenStations = async (devices, stations) => {
    ...
    Object.values(devices).map(async (device) => {
      if (
        !!device.station_id &&
        !Object.keys(stations).includes(device.station_id)
      ) {
        console.log(
          "QQQQ Device has a station ID that needs to be deleted",
          device
        );
  ...
  const handleDashboardsWithBrokenStations = (dashboards, stations) => {
    ...
    Object.values(dashboards).map((dashboard) => {
      if (
        !!dashboard.location &&
        !dashboard.device &&
        !stations[dashboard.location]
      ) {
        console.log(
          "QQQQ dashboard belongs to a station that does not exist",
          dashboard
        );
    ...
    Object.values(processes).map((process) => {
      ...
      process.routes.map(async (route) => {
        ...
        if (!tasks[route]) {
          ...
          console.log(
            "QQQQ route does not exist in anymore, delete from process",
            deepCopy(updatedProcess)
          );
      ...
      if (task.processes.length > 0) {
        task.processes.map((process) => {
          ...
          if (!processes[process]) {
            ...
            console.log(
              "QQQQ Process does not exist anymore, removing from task",
              task
            );
            ...
            console.log(
              "QQQQ Task is associated with a process that is not associated with the task anymore",
              task,
              process
            );
            alert(
              "Task is associated with a process that is not associated with the task anymore, adding back to process"
            );
  ...
  return <SplashScreen isApiLoaded={props.isApiLoaded} apiError={apiError} />;
};
