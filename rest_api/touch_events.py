...
def close_touch_event(touch_event):
    ...
    if touch_event['load_station_id'] == 'QUEUE':
        ...
        [pgs_cycle_time, pgs_historical_cycle_time] = calculate_pg_station_cycle_time(touch_event['load_station_id'], touch_event['product_group_id'], touch_event)
    ...
    lot_arrival_events = list(collection.find({'lot_id': touch_event['lot_id'], 'unload_station_id': touch_event['load_station_id']}).sort('move_datetime'))
    if len(lot_arrival_events) > 0:
        
        ave_arrival_time = sum([e['quantity']*datetime.timestamp(e['move_datetime']) for e in lot_arrival_events]) / sum([e['quantity'] for e in lot_arrival_events])
        
        lot_departure_events = list(collection.find({'$and': [{'lot_id': touch_event['lot_id']}, {'load_station_id': touch_event['load_station_id']}]}).sort('move_datetime'))
        for lot_departure_event in lot_departure_events:
            idle_seconds = working_seconds_between_datetimes(shift_details, ave_arrival_time, lot_departure_event['move_datetime'])
            collection.update_one({'_id': lot_departure_event['_id']}, {'$set': {'idle_seconds': round(idle_seconds)}})
    ...
    if touch_event['load_station_id'] != 'QUEUE':
        ...
        for station_id in process_station_ids:
            generate_station_summaries_until_date(station_id, touch_event['move_datetime'])
...
def full_create(touch_event):
    ...
    # add to collection
    touch_event['start_datetime'] = datetime.fromtimestamp(touch_event['start_datetime'] / 1000)
    touch_event['move_datetime'] = datetime.fromtimestamp(touch_event['move_datetime'] / 1000)
    touch_event['working_seconds'] = working_seconds_between_datetimes(shift_details,touch_event['move_datetime'], touch_event['start_datetime'])
    touch_event['idle_seconds'] = 0
    ...
    if len(lot_arrival_events) > 0:
        ...
        ave_arrival_time = datetime.fromtimestamp(sum([e['quantity']*datetime.timestamp(e['move_datetime']) for e in lot_arrival_events]) / sum([e['quantity'] for e in lot_arrival_events]))
...
def get_open_touch_events_for_station(station_id):
    ...
    for i in range(len(open_touch_events)):
        ...
        open_touch_events[i]['start_datetime'] = datetime.timestamp(open_touch_events[i]['start_datetime']) * 1000
    
    return dumps(open_touch_events)

def get_open_touch_events(map_id):
