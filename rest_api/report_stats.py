...
def get_stats(station_id, info, output=False):
    ...
    if info['timespan'] == 'day':
        data['reports'], data['date_title'] = calc_day_stats(station_id, index=info['index'], output=output)
    elif info['timespan'] == 'week':
        data['reports'], data['date_title'] = calc_week_stats(station_id, index=info['index'], output=output)
    elif info['timespan'] == 'month':
        data['reports'], data['date_title'] = calc_6_week_stats(station_id, index=info['index'], output=output)
    else:
        data['reports'], data['date_title'] = calc_year_stats(station_id, index=info['index'], output=output)
...
#     end_utc = time.mktime(end_of_time_frame.timetuple())
    ...
    rtn_data = create_data(station_id, start_utc, end_utc, lables, output=output)
