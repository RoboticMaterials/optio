...
def delete_all_process_summaries():
...
def generate_process_statistics(process_id, start_date, end_date=None):
    ...
    if end_date is None:
        ...
        stats = generate_process_statistics_from_events(process_id, min_datetime, max_datetime)
        ...
        process_station_ids = [node['stationID'] for node in process['flattened_stations']]
        ...
        if max_datetime.date() == datetime.now().date():
            ...
            temp_summary_start = datetime.combine(max_datetime, datetime.min.time())
            for station_id in process_station_ids:
                station = db.stations.find_one({'_id': station_id})
                print('    TEMP STATION SUMMARY FOR {}'.format(station['name']))
            ...
            temp_process_summary = summarize_process_events(process, temp_station_summaries, datetime.combine(max_datetime.date(), datetime.min.time()), max_datetime)
            summaries.append(temp_process_summary)
        
        print('Generating process statistics from {} summaries ({} -> {})'.format(len(summaries), min_datetime, max_datetime))
    
        stats = generate_process_statistics_from_summaries(summaries, process_id)
...
def generate_process_statistics_from_events(process_id, min_datetime, max_datetime):
    ...
    print('Generating process stats from {} touch events ({})'.format(len(touch_events), min_datetime.date()))
    ...
    process_stations_nrml = {station['_id']: station for station in db.stations.find({'_id': {'$in': process_station_ids}})}
    ...
    [day_start, day_end] = shift_on_date(min_datetime, shift_start_time, shift_end_time)
    start_hour = round_time(min([touch_events[0]['move_datetime'], day_start]), timedelta(hours=1), 'floor')
    end_hour = round_time(max([touch_events[-1]['move_datetime'], day_end]), timedelta(hours=1), 'ceil') - timedelta(seconds=1)
    ...
    working_seconds = working_seconds_between_datetimes(shift_details, min_datetime, max_datetime)
    
    # --- Production Rate
    production_rates = {}
    total_throughputs = {}
    for pg_id in product_group_ids:
        pg_finished_qty = safe_int(touches_df[(touches_df['product_group_id'] == pg_id) & (touches_df['unload_station_id'].isin(end_station_ids))]['quantity'].sum())
        total_throughputs[pg_id] = pg_finished_qty
        production_rates[pg_id] = safe_int(safe_divide(working_seconds, pg_finished_qty))
    total_finished_qty = safe_int(touches_df[touches_df['unload_station_id'].isin(end_station_ids)]['quantity'].sum())
    ...
    current_wip = {}
    for station_id in process_station_ids:
        latest_station_events = list(db.touch_events.find({'load_station_id': station_id, 'move_datetime': {'$lte': max_datetime}}).sort('move_datetime', -1).limit(1))
    ...
    # --- Lead Time
    stats['lead_time'] = safe_divide(stats['production_rates']['total'] * stats['wip']['total'] * 86400, daily_working_seconds) # The ratio of how much of the day (86400 seconds) is working seconds
    ...
    for pg_id in product_group_ids:
        ...
        for station_id in process_station_ids:
            ...
            latest_pg_station_event = list(db.touch_events.find({'product_group_id': pg_id, 'load_station_id': station_id, 'move_datetime': {'$lte': max_datetime}}).sort('move_datetime', -1).limit(1))
    ...
    # --- Throughput
    out_touches_df = touches_df[touches_df['unload_station_id'].isin(end_station_ids)]
...
def generate_next_process_summary(process_id, current_datetime=None):
    ...
    latest_summaries = list(db.process_summaries.find({'process_id': process_id}).sort([("datetime",-1)]).limit(1))
    if len(latest_summaries) == 0:
        first_station_summary_datetime = None
        for station_id in process_station_ids:
            first_summaries = list(db.station_summaries.find({'station_id': station_id}).sort('datetime', 1).limit(1))
        ...
        next_summary_start_datetime = datetime.combine(first_station_summary_datetime.date(), datetime.min.time())
    else:
    ...
    if current_datetime >= next_summary_end_datetime:  # If last summary was more then the summary period ago, make a new summary
        ...
        print('Generating process summary for process "{}" on {} with {} station summaries'.format(process['name'], next_summary_start_datetime, len(station_summaries)))
        ...
        print('No summary to be made', current_datetime, next_summary_end_datetime)
        return None
...
def summarize_process_events(process, station_summaries, start_datetime, end_datetime):
    ...
    process_station_ids = [node['stationID'] for node in process['flattened_stations']]
    process_stations_nrml = {station['_id']: station for station in db.stations.find({'_id': {'$in': process_station_ids}})}
    process_stations = [process_stations_nrml[station_id] for station_id in process_station_ids] # Ensures we get the correct order
    
    process_summary['station_ids'] = process_station_ids
    process_summary['routes'] = [{'load': route['load'], 'unload': route['unload']} for route in process_routes]
    
    # ===== Shift Information
    shift_details = db.settings.find_one()['shiftDetails']
    working_seconds = working_seconds_between_datetimes(shift_details, start_datetime, end_datetime)
    process_summary['working_seconds'] = working_seconds
    
    # ===== Convert everything to dataframes (rows are each summary, columns are pg_ids)
    datetimes_idx = pd.Index([station_summary['datetime'] for station_summary in station_summaries])
    
    summaries_df = pd.DataFrame(station_summaries)
    station_throughput_df = pd.DataFrame.from_dict([station_summary['throughput'] for station_summary in station_summaries]).fillna(0).set_index(datetimes_idx)
    wip_df = pd.DataFrame.from_dict([station_summary['wip'] for station_summary in station_summaries]).fillna(0).set_index(datetimes_idx)
    
    # ===== Neccessary Database Information ====== #
    product_group_ids = list(set.union(set(wip_df.columns.tolist()), set(station_throughput_df.columns.tolist())))
    product_groups = list(db.lot_templates.find({'_id': {'$in': product_group_ids}}))
    product_groups = {product_group['_id']: product_group for product_group in product_groups if product_group['processId'] == process['_id']} # Map to normalized object
    product_group_ids = [product_group['_id'] for product_group in product_groups.values()] # Only product groups that are part of that process
    ...
    for station_summary in station_summaries:
        summary_outputs = sum_across_dicts([out_dict for station_id, out_dict in station_summary['out_stations']['data'].items() if station_id in end_station_ids])
        if station_summary['datetime'].date() == last_summary_date: # Multiple finish stations on the same day
    ...
    output_df = pd.DataFrame.from_dict(outputs).fillna(0).set_index(datetimes_idx.unique())
    ...
    for pg_id in product_group_ids:
        ...
        production_rates[pg_id] = safe_int(safe_divide(working_seconds, finished_quantity))
    ...
    for station in process_stations:
        latest_station_events = list(db.touch_events.find({'load_station_id': station['_id'], 'product_group_id': {'$in': product_group_ids}, 'move_datetime': {'$lte': end_datetime}}).sort('_id', -1).limit(1))
        if len(latest_station_events) > 0:
            for pg_id, qty in latest_station_events[0]['current_wip'].items():
                if pg_id in current_wip:
                    current_wip[pg_id] += qty
    ...
    for station_id in process_station_ids:
        station_cycle_times = {}
        station_cycle_time_df = pd.DataFrame.from_dict([ct_dict for ct_dict in summaries_df[(summaries_df['station_id'] == station_id)]['cycle_times']])
        for pg_id in product_group_ids:
            if pg_id in station_cycle_time_df: 
                station_cycle_times[pg_id] = safe_int(station_cycle_time_df[pg_id].mean())
...
def generate_process_statistics_from_summaries(summaries, process_id):
    ...
    if (len(summaries)) == 0: return stats
    ...
    production_rates_df = pd.DataFrame.from_dict([summary['production_rates'] for summary in summaries]).set_index(datetimes_idx)
    throughputs_df = pd.DataFrame.from_dict([summary['throughputs'] for summary in summaries]).set_index(datetimes_idx)
    wip_df = pd.DataFrame.from_dict([summary['wip'] for summary in summaries]).set_index(datetimes_idx)
    
    product_group_ids = list(set.union(set(wip_df.columns.tolist()), set(throughputs_df.columns.tolist())))
    product_groups = list(db.lot_templates.find({'_id': {'$in': product_group_ids}}))
    product_groups = {product_group['_id']: product_group for product_group in product_groups} # Map to normalized object
    ...
    for pg_id in production_rates_df:
        ...
        if pg_id == '__total':
            stats['production_rates']['total'] = safe_int(production_rates_df[pg_id].mean())
        else:
    ...
    # --- Lead Time    
    stats['lead_time'] = stats['production_rates']['total'] * stats['wip']['total'] * 86400 / daily_working_seconds # The ratio of how much of the day (86400 seconds) is working seconds
    
    balance = {}
    for station_id in process_station_ids:
        station_cycle_time_df = pd.DataFrame.from_dict([summary['cycle_times'][station_id] for summary in summaries if station_id in summary['cycle_times']])
        station = process_stations_nrml[station_id] if station_id in process_stations_nrml else None
        for pg_id in product_group_ids:
            if pg_id not in product_group_ids: continue
            if pg_id == '__total': continue
...
def calculate_process_production_rate(process_id):
    ...
    events = list(db.touch_events.find({'unload_station_id': {'$in': end_station_ids}}).sort('move_datetime').sort('move_datetime', -1).limit(30))
    ...
    total_working_seconds = working_seconds_between_datetimes(shift_details, first_event_time, last_event_time)
    
    total_finished_qty = safe_int(events_df['quantity'][1:].sum()) # Since we use the first events move time to start the timer, dont include it's quantity
    production_rate = safe_int(safe_divide(total_working_seconds, total_finished_qty))
