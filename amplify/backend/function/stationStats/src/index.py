...
dynamodb = resource(service_name='dynamodb', region_name=os.environ['REGION'], verify=False, endpoint_url=os.environ["DDB_ENDPOINT"], aws_access_key_id=os.environ["AWS_ACCESS_KEY_ID"], aws_secret_access_key=os.environ["AWS_SECRET_ACCESS_KEY"])

# table def parameters of events for stations
table = dynamodb.Table(os.environ['API_RMSTUDIOCLOUD_STATIONEVENTTABLE_NAME'])

def handler(event, context):
    print("os.environ['API_RMSTUDIOCLOUD_STATIONEVENTTABLE_NAME']",os.environ['API_RMSTUDIOCLOUD_STATIONEVENTTABLE_NAME'])
