...
def get_stats(stationId, info, tz, output=False):
    ...
    if info['timespan'] == 'day':
        data['reports'], data['date_title'] = calc_day_stats(stationId, tz, index=info['index'], output=output)
    elif info['timespan'] == 'week':
        data['reports'], data['date_title'] = calc_week_stats(stationId, tz, index=info['index'],output=output)
    elif info['timespan'] == 'month':
        data['reports'], data['date_title'] = calc_6_week_stats(stationId, tz, index=info['index'],output=output)
    else:
        data['reports'], data['date_title'] = calc_year_stats(stationId, tz, index=info['index'],output=output)
    return data
...
def create_data(stationId, start_utc, end_utc, labels, tz, sort_key='object', output=False):
    ...
    if len(df) > 0:
        ...
        print(df['reportButtonId'])

        # Find unique ids
        unique_ids = df['reportButtonId'].unique()   
