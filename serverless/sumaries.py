...
def generate_next_summary(collection, summary_period, current_dateTime=None):
    ...
    floored_latest_summary_dateTime = round_time(
        latest_summary_dateTime, delta=summary_period, to='floor')
    ...
    if current_dateTime > next_summary_dateTime:
...
def day_historical_summaries(current_dateTime=None):
    ...
    summary = generate_next_summary(
        db.historicals_day, timedelta(days=1), current_dateTime)
