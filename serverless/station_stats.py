...
def generate_station_statistics(station_id, start_date, end_date=None):
    ...
    if end_date is None:
        ...
        print('Generating stats from {} touch events and {} report events ({})'.format(
            len(touch_events), len(report_events), min_datetime.date()))
        stats = generate_station_statistics_from_events(
            station_id, touch_events, report_events, start_date)
        ...
        print('Generating stats from {} summaries ({} -> {})'.format(len(summaries),
              min_datetime.date(), max_datetime.date()))
        stats = generate_station_statistics_from_summaries(
            station_id, summaries)
...
def bin_wip_to_lines_data(initial_wip, df, bins, product_groups):
    ...
    # If there are no events in a bin, we will default to the last known wip values
    last_wip_values_df = pd.DataFrame.from_dict([initial_wip])
    for bin_data in grouped_data:
        ...
        for pg in product_groups:
            if pg['_id'] in mean_wip:
                wip_data_dict[pg['_id']]['data'].append(
                    {'x': datetime.timestamp(bin_data[0].right)*1000, 'y': mean_wip[pg['_id']]})
            else:
                wip_data_dict[pg['_id']]['data'].append(
                    {'x': datetime.timestamp(bin_data[0].right)*1000, 'y': 0})
...
def product_group_pie(df, product_groups, processes):
    ...
    for pg_id, pg_df in grouped_df:
        ...
        product_group_process_name = processes[product_groups[pg_id]['processId']]['name'] if (
            pg_id in product_groups and product_groups[pg_id]['processId'] in processes) else '???'
...
def bin_reports_to_bar_data(df, bins):
    ...
    for data in binned_data:
        bar = {
            # X value is the timestep to the right of the bin
            "x": data[0].right
        }
...
def shift_on_date(date, shift_start_time, shift_end_time):
    start = date.replace(hour=shift_start_time.hour,
                         minute=shift_start_time.minute, second=0, microsecond=0)
    end = date.replace(hour=shift_end_time.hour,
                       minute=shift_end_time.hour, second=0, microsecond=0)
    return [start, end]
...
def generate_station_statistics_from_events(station_id, touch_events, report_events, start_date):
    ...
    [day_start, day_end] = shift_on_date(
        start_date, shift_start_time, shift_end_time)
    ...
    end_hour = round_time(day_end, timedelta(hours=1),
                          'ceil') - timedelta(seconds=1)
    ...
    if te_count > 0:
        ...
        # Map to normalized object
        processes = {process['_id']: process for process in processes}

        stations = list(db.stations.find({'map_id': station['map_id']}))
        # Map to normalized object
        stations = {station['_id']: station for station in stations}
        ...
        for pg_id, grouped_events in pg_grouped_data:
            ...
            stats['partials'][pg_id] = {
                'value': (pgs_events_count / pgs_ct_sum), 'quantity': pg_qty}
            # stats['partials'][pg_id] = {'value': (1 / grouped_events['pgs_cycle_time'][-1]), 'quantity': pg_qty}

        stats['total_quantity'] = total_qty

        found_pg_ids = []
        # Reverse order because we want to take the stats at the end of the day
        for touch_event in touch_events[::-1]:
            ...
            if pg_id not in found_pg_ids:
                ...
                product_group_process_name = processes[product_groups[pg_id]['processId']]['name'] if (
                    product_groups[pg_id]['processId'] in processes) else '???'
        ...
        for pg_id, data in stats['cycle_time'].items():
            ave = np.mean([datapoint['y']
                          for datapoint in data['line_data'][0]['data']])
        ...
        initial_wip = initial_wip_events[0]['current_wip'] if len(
            initial_wip_events) else {}

        stats['wip'] = bin_wip_to_lines_data(
            initial_wip, touches_df, bins, product_groups.values())

        # --- Product Group Pie
        stats['product_group_pie'] = product_group_pie(
            touches_df, product_groups, processes)
        ...
        # --- Machine Utilization
        stats['machine_utilization'] = machine_utilization(
            touches_df, working_seconds)
    ...
    if re_count > 0:
        ...
        report_buttons_normalized = {
            x['_id']: x for x in dashboard['report_buttons']} if 'report_buttons' in dashboard else None

        # Also store a dictionary of all colors, with the key being the report name
        reports_colors = {}
        for i, re in enumerate(report_events):
            rb_id = re['report_button_id']
            if rb_id in report_buttons_normalized:
                report_name = report_buttons_normalized[rb_id][
                    'label'] if report_buttons_normalized is not None else '???'
                report_events[i]['report_name'] = report_name
                reports_colors[report_name] = report_buttons_normalized[rb_id]['color'] if report_buttons_normalized is not None else '#{}'.format(
                    str(np.random.randint(0, 10))*6)  # Random grey color if dashboard doesnt exist
...
def generate_next_station_summary(station_id, current_datetime=None):
    ...
    latest_summaries = list(db.station_summaries.find(
    ...
    if len(latest_summaries) == 0:
        ...
        first_touch_events = list(db.touch_events.find({'load_station_id': station_id, 'move_datetime': {
                                  '$ne': None}}).sort([("move_datetime", 1)]).limit(1))
        first_report_events = list(db.report_events.find(
            {'station_id': station_id}).sort([("datetime", 1)]).limit(1))
        ...
        next_summary_start_datetime = datetime.combine(
            first_event_datetime.date(), datetime.min.time())
    else:
    ...
    if current_datetime >= next_summary_end_datetime:
        ...
        station = db.stations.find_one({'_id': station_id})
        print('Generating station summary for station "{}" on {}'.format(
            station['name'], next_summary_start_datetime))
...
def summarize_station_events(touch_events, report_events, station, summary_datetime):
    ...
    if len(touch_events):
        ...
        pg_grouped_data = touches_df.groupby(['product_group_id'])
        lastest_pg_events = touches_df.drop_duplicates(
            'product_group_id', keep='last')
        ...
        for pg_id, grouped_events in pg_grouped_data:
            ...
            pgs_count_over_ct[pg_id] = safe_divide(
                pgs_events_count, pgs_ct_sum)
        ...
        for process_id, grouped_events in touches_df.groupby(['process_id']):
            process_pie[process_id] = safe_int(
                grouped_events['quantity'].sum())
        ...
        for unload_station_id, grouped_station_events in touches_df.groupby(['unload_station_id']):
            ...
            for pg_id, grouped_pg_events in grouped_station_events.groupby(['product_group_id']):
                out_stations[unload_station_id][pg_id] = safe_int(
                    grouped_pg_events['quantity'].sum())
        ...
        for operator_name, grouped_events in touches_df.groupby(['operator']):
            operator_pie[operator_name] = safe_int(
                grouped_events['quantity'].sum())
        ...
        wip = {key: safe_int(values_df[key].mean())
        ...
        summary['total_working_seconds'] = safe_int(
            touches_df['working_seconds'].sum())
        summary['total_seconds'] = working_seconds
        summary['mean_working_seconds'] = safe_int(
            (touches_df['working_seconds'] / touches_df['quantity']).mean())
        summary['mean_idle_seconds'] = safe_int(
            (touches_df['idle_seconds']).sum())
...
def generate_station_statistics_from_summaries(station_id, summaries):
    ...
    pgs_count_over_ct_df = pd.DataFrame.from_dict(
        [summary['pgs_count_over_ct'] for summary in summaries])
    ...
    cycle_time_df = pd.DataFrame.from_dict(
        [summary['cycle_times'] for summary in summaries]).set_index(datetimes_idx)
    throughput_df = pd.DataFrame.from_dict(
        [summary['throughput'] for summary in summaries]).fillna(0).set_index(datetimes_idx)
    reports_df = pd.DataFrame.from_dict(
        [summary['reports'] for summary in summaries])
    wip_df = pd.DataFrame.from_dict(
        [summary['wip'] for summary in summaries]).fillna(0).set_index(datetimes_idx)
    processes_df = pd.DataFrame.from_dict(
        [summary['processes'] for summary in summaries])
    product_groups_df = pd.DataFrame.from_dict(
        [summary['product_groups'] for summary in summaries])
    out_stations_df = pd.DataFrame.from_dict(
        [summary['out_stations']['totals'] for summary in summaries])
    operators_df = pd.DataFrame.from_dict(
        [summary['operators'] for summary in summaries])

    # Dict with key=product group ID and value is the total quantity moved for that pg
    total_quantities = {
        pg_id: throughput_df[pg_id].sum() for pg_id in throughput_df}
    ...
    product_group_ids = list(
        set.union(set(wip_df.columns.tolist()), set(throughput_df.columns.tolist())))
    product_groups = list(db.lot_templates.find(
        {'_id': {'$in': product_group_ids}}))
    # Map to normalized object
    product_groups = {
    ...
    for pg_id in pgs_count_over_ct_df:
        ...
        partials[pg_id] = {'value': pgs_count_over_ct_df[pg_id].mean(
        ), 'quantity': total_quantities[pg_id]}
    ...
    for summary in summaries:
        if summary['total_events'] == 0:
            if last_summary_wip is None:  # Must have an initial wip value. If the first summary doesnt have any events, then take the last summary before it that has events
                ...
                last_summary_wip = last_summary_wip_summaries[0]['wip'] if len(
                    last_summary_wip_summaries) else {}
    ...
    if 'report_buttons' in dashboard:
        report_buttons_normalized = {
            x['_id']: x for x in dashboard['report_buttons']}
        ...
        stats['reports_pie'] = [{
            ...
            # If no button color, assign to random grey
            'color': report_buttons_normalized[report_button_id]['color'] if report_button_id in report_buttons_normalized else str(np.random.randint(0, 10))*6
    ...
    # --- Machine Utilization
    machine_working_seconds = sum(
        [summary['total_working_seconds'] for summary in summaries])
...
def calculate_pg_station_cycle_time(station_id, product_group_id, new_event):
    base_query = {'load_station_id': station_id,
                  'product_group_id': product_group_id, 'move_datetime': {'$ne': None}}

    recent_events = list(db.touch_events.find(
        base_query).sort('move_datetime', -1).limit(29))
    recent_events.append(new_event)
    first_event = list(db.touch_events.find(
        base_query).sort('move_datetime', 1).limit(1))

    if len(recent_events) > 2:
        first_event = first_event[0]

        [recent_quantity, recent_working_seconds] = aggregate(
            base_query, limit=29)
        [total_quantity, total_working_seconds] = aggregate(
        ...
        ps_historical_cycle_time = round(
            safe_divide(total_working_seconds, total_quantity))
        ps_cycle_time = round(safe_divide(
            recent_working_seconds, recent_quantity))
...
def working_seconds_between_datetimes(shift_details, start, end):
    ...
    if start.date() == end.date():
        ...
        first_day_working_seconds = calc_daily_working_seconds(
            shift_details, start.time(), None)
        last_day_working_seconds = calc_daily_working_seconds(
            shift_details, None, end.time())

        daily_working_seconds = calc_daily_working_seconds(
            shift_details, None, None)
        between_business_days = max(
            0, np.busday_count(start.date(), end.date()) - 1)
...
def round_time(dt=None, delta=timedelta(minutes=1), to='average'):
    ...
    """
    minDt = datetime.now().replace(month=1, day=1, hour=0,
...
def calc_daily_working_seconds(shift_details, start_time=None, stop_time=None):
    ...
    begin_shift_secs = timedelta(
        hours=begin_shift.tm_hour, minutes=begin_shift.tm_min, seconds=begin_shift.tm_sec).total_seconds()

    # Cant start before the shift starts
    if start_time is not None:
        start_time_secs = timedelta(
            hours=start_time.hour, minutes=start_time.minute, seconds=start_time.second).total_seconds()
    ...
    end_of_shift_secs = timedelta(
        hours=end_of_shift.tm_hour, minutes=end_of_shift.tm_min, seconds=end_of_shift.tm_sec).total_seconds()

    if stop_time is not None:
        end_time_secs = timedelta(
            hours=stop_time.hour, minutes=stop_time.minute, seconds=stop_time.second).total_seconds()
    ...
    for br in breaks:
        if(br['enabled']):
            ...
            br_start_secs = timedelta(
                hours=br_start_time.tm_hour, minutes=br_start_time.tm_min, seconds=br_start_time.tm_sec).total_seconds()
            br_start_secs = max(start_time_secs, min(
                br_start_secs, end_time_secs))

            br_end_time = time.strptime(br['endOfBreak'], '%H:%M')
            br_end_secs = timedelta(
                hours=br_end_time.tm_hour, minutes=br_end_time.tm_min, seconds=br_end_time.tm_sec).total_seconds()
