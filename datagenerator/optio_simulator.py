...
#optio = Optio('nikolaus@optio.cloud', 'Vaygts99', instance='dev.optio.cloud', protocol="https")
...
def create_card(map_id, process_id, template_id, station_id, quantity=1):
     ...
     name = StringGenerator('Order ${SKU}, ${name}, ${color}').render(SKU=list(range(10000,20000)),name=['Hurricane','Tempest','Storm'],color=['Large','Small','X-Large'])
...
if reset:
    ...
    start_datetime = round_to_shift_start(datetime.now() - timedelta(days=20))
    print('Starting Simulation at {}'.format(start_datetime))
    start_UTC = datetime.timestamp(start_datetime)

    mcards = 20          # Mean number of cards per station
